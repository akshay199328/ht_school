...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	{
		$usersSql  = "SELECT users.ID, users.user_login, users.user_nicename, users.user_email, users.display_name, meta.user_id ";
		$usersSql .= "FROM ht_users AS users ";
		$usersSql .= "INNER JOIN ht_usermeta AS meta ON users.ID = meta.user_id ";
		$usersSql .= "WHERE meta.meta_key = 'ht_capabilities' AND meta.meta_value LIKE '%school%' AND users.ID >= '".$_POST['user_id']."' ";
...
//print_r($usersList);die;
		...
		$fileName		= "schools_".date("Y_m_d_H_i").".csv";
		$fieldHeaders	= array( "school_name", "school_id");
		$actualData		= array();
...
<body>
	<form method="post">
		<input type="number" name="user_id" placeholder="Pleas Enter school ID">
