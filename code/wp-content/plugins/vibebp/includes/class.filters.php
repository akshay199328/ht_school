...
class VibeBP_Filters{
	...
	private function __construct(){
		...
		add_filter('the_content',function($content){
			...
			global $post;
            if(!empty($post) && strpos($content, 'vibe_editor_rich_text') && ($post->post_type == 'unit' || $post->post_type == 'quiz' ) && !is_singular('unit')){
    ...
    function bp_admin_nav($admin_nav){
		...
		if(vibebp_get_setting('bp_single_page')){
			foreach($admin_nav as $k=>$nav){
				...
				$admin_nav[$k]['href']=str_replace(bp_core_get_user_domain( get_current_user_id() ), get_permalink(vibebp_get_setting('bp_single_page')).'#component=', $nav['href']);

				preg_match('/.+#component=(.+)\/(.+)/', $admin_nav[$k]['href'],$matches);
				$admin_nav[$k]['href']=get_permalink(vibebp_get_setting('bp_single_page')).'#component='.$matches[1];
    ...
    function check_country_field_display($value,$field){
    	if(!empty($field) && $field->type=='country'){
