jQuery(document).ready(function($){
	...
	if($('body').hasClass('settings_page_wpf-settings')) {
		...
		var startBatch = function( button, action, args = false ) {
			...
			button.html('<span class="dashicons dashicons-update-alt spin"></span>' + wpf_ajax.strings.beginningProcessing.replace( 'ACTIONTITLE', action.title ));
		...
		var syncTags = function(button, total, crmContainer) {
			...
			button.find('span.dashicons').addClass('spin');
			...
			$.post(ajaxurl, data, function(response) {
				...
				if(response.success == true) {
					...
					if( $('#connection_configured').val() == true || $('#connection_configured').val() == "1" || $('#connection_configured').val() == "true" ) {
						...
						button.find('span.dashicons').removeClass('spin');
		...
		$( "#edd-license" ).on( "click", function() {
			$(this).html('<span class="dashicons dashicons-update-alt spin"></span> Connecting'); 
	        $(this).attr('disabled', 'disabled');
