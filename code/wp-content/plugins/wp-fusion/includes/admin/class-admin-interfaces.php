...
class WPF_Admin_Interfaces {
	...
	public function restrict_manage_users() {
		...
		$filter_options = array(
			false     => __( 'Filter By Tag', 'wp-fusion' ),
	...
	public function taxonomy_form_fields( $term ) {
		...
		<table class="wpf-settings-table form-table wpf-meta">
			...
			<tbody>
				...
				<tr class="form-field">
					<th style="padding-bottom: 0px;" colspan="2"><h3 style="margin: 0px;"><?php _e( 'WP Fusion - Access Settings', 'wp-fusion' ); ?></h3></th>
				</tr>
				...
				<tr class="form-field">
					...
					<td>
						...
						<span class="description"><?php printf( __( 'Apply these restrictions to all posts in the %1$s %2$s.', 'wp-fusion' ), $term->name, $taxonomy->labels->singular_name ); ?></p>
					</td>
				</tr>
				...
				<tr class="form-field">
					...
					<td>
						...
						<span class="description"><?php _e( 'The taxonomy term will be completely hidden from all term listings. (Note that this just hides the term itself. To completely hide all restricted posts enable Filter Queries in the WP Fusion settings)', 'wp-fusion' ); ?></p>
					</td>
				</tr>
				...
				<tr class="form-field">
					...
					<td style="max-width: 400px;">
						...
						<span class="description"><?php printf( __( 'Apply these tags when any post in this %s is viewed', 'wp-fusion' ), $taxonomy->labels->singular_name ); ?></span>
	...
	public function manage_users_columns( $columns ) {
		...
		$columns['wpf_tags'] = sprintf( __( '%s Tags', 'wp-fusion' ), wp_fusion()->crm->name );
		...
		<div id="wpf-meta" class="inline-edit-col-wpf">
			<div class="inline-edit-col">
				<div style="margin: 10px">
					<?php $this->restrict_content_checkbox( $post, WPF_Admin_Interfaces::$meta_box_defaults ); ?>
				</div>
				<?php $this->required_tags_select( $post, WPF_Admin_Interfaces::$meta_box_defaults ); ?>
				<?php $this->page_redirect_select( $post, WPF_Admin_Interfaces::$meta_box_defaults ); ?>
				<?php $this->external_redirect_input( $post, WPF_Admin_Interfaces::$meta_box_defaults ); ?>
		...
		<div class="wpf_nav_menu_field description-wide" style="margin: 5px 0;">
			...
			<label for="wpf_nav_menu-for-<?php echo $item->ID; ?>">
				...
				<select name="wpf-nav-menu[<?php echo $item->ID; ?>][lock_content]" id="wpf_nav_menu-for-<?php echo $item->ID; ?>" class="wpf-nav-menu">
					...
					<option value="1" <?php selected( true, $settings['lock_content'] ); ?> ><?php _e( 'Logged In Users', 'wp-fusion' ); ?></option>
					<option value="loggedout" <?php selected( 'loggedout', $settings['lock_content'] ); ?> ><?php _e( 'Logged Out Users', 'wp-fusion' ); ?></option>
	 ...
	 * Meta box notice.
	 *
	 * Output a notice when the content's visibility is affected by a restricted
	 * taxonomy term.
	 *
	 * @since 3.36.2
	 *
	...
	public function meta_box_notice( $post, $settings ) {
		...
		foreach ( $terms as $term ) {
			...
			if ( empty( $taxonomy_rules[ $term->term_id ] ) || ! isset( $taxonomy_rules[ $term->term_id ]['lock_posts'] ) ) {
				continue;
			}

			if ( ! empty( $taxonomy_rules[ $term->term_id ]['allow_tags'] ) ) {
				$access_terms[] = $term->name;
				$required_tags  = array_merge( $required_tags, $taxonomy_rules[ $term->term_id ]['allow_tags'] );
			}
		}
		...
		printf( __( 'This %1$s is inheriting access rules from taxonomy terms: %2$s.', 'wp-fusion' ), $post_type_object, '<strong>' . implode( ', ', $access_terms ) . '</strong>' );
	...
	public function restrict_content_checkbox( $post, $settings ) {
		...
		printf( __( 'Users must be logged in to view this %s', 'wp-fusion' ), $post_type_object_label );
		echo '</label>';
	...
	public function required_tags_select( $post, $settings ) {
		...
		echo '<span class="dashicons dashicons-editor-help wpf-tip wpf-tip-bottom" data-tip="' . __( 'The user must be logged in and have at least one of the tags specified to access the content.', 'wp-fusion' ) . '"></span></label>';
		...
		echo '<span class="dashicons dashicons-editor-help wpf-tip wpf-tip-bottom" data-tip="' . __( 'The user must be logged in and have <em>all</em> of the tags specified to access the content.', 'wp-fusion' ) . '"></span></label>';
		...
		echo '<span class="dashicons dashicons-editor-help wpf-tip wpf-tip-bottom" data-tip="' . __( 'If the user is logged in, they must have <em>none</em> of the tags specified to access the content.', 'wp-fusion' ) . '"></span></label>';
	...
	public function page_redirect_select( $post, $settings, $disabled = false ) {
		...
		$available_posts = array(
			'Misc.' => array(
				'home'  => __( 'Home Page', 'wp-fusion' ),
				'login' => __( 'Login Page', 'wp-fusion' ),
			),
		);
		...
		echo '<span class="dashicons dashicons-editor-help wpf-tip wpf-tip-bottom" data-tip="' . __( 'If you do not specify a redirect WP Fusion will try to replace the content area of the post with the restricted content message configured in the WP Fusion settings.', 'wp-fusion' ) . '"></span></label>';

		echo '<select ' . ( $disabled ? 'disabled' : '' ) . ' id="wpf-redirect" class="select4-search" style="width: 100%;" data-placeholder="' . __( 'Show restricted content message', 'wp-fusion' ) . '" name="wpf-settings[redirect]">';
	...
	public function apply_tags_select( $post, $settings ) {
		...
		echo '<p class="wpf-apply-tags-select"><label for="wpf-apply-tags"><small>' . sprintf( __( 'Apply tags when a user views this %s', 'wp-fusion' ), $post_type_object_label ) . ':</small></label>';
		...
		echo '<p class="wpf-apply-tags-select"><label for="wpf-remove-tags"><small>' . sprintf( __( 'Remove tags when a user views this %s', 'wp-fusion' ), $post_type_object_label ) . ':</small></label>';
