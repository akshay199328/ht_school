...
class WPF_User_Profile {
	...
	public function user_profile( $user ) {
		...
		?>
		<h3><?php _e( 'WP Fusion', 'wp-fusion' ); ?></h3>
		...
		<table class="form-table">
			...
			<tr>
				...
				<td id="contact-id">
					<?php if ( $contact_id = wp_fusion()->user->get_contact_id( $user->ID ) ) : ?>
						...
						<?php else : ?>
							...
							<?php if ( isset( wp_fusion()->crm->edit_url ) ) : ?>
								...
								- <a href="<?php printf( wp_fusion()->crm->edit_url, $contact_id ); ?>" target="_blank"><?php printf( __( 'View in %s', 'wp-fusion' ), wp_fusion()->crm->name ); ?> &rarr;</a>
					...
					<?php else : ?>
						...
						<?php printf( __( 'No %s contact record found.', 'wp-fusion' ), wp_fusion()->crm->name ); ?>

						<a href="<?php echo admin_url( 'user-edit.php' ); ?>?user_id=<?php echo $user->ID; ?>&wpf_profile_action=register"><?php _e( 'Create new contact', 'wp-fusion' ); ?>.</a>
			...
			<tr>
				<th><label for="resync_contact"><?php _e( 'Additional Actions', 'wp-fusion' ); ?></label></th>
				<td>

					<a href="<?php echo admin_url( 'user-edit.php' ); ?>?user_id=<?php echo $user->ID; ?>&wpf_profile_action=push"><?php _e( 'Push User Meta', 'wp-fusion' ); ?></a>

					<span class="dashicons dashicons-editor-help wpf-tip wpf-tip-bottom" data-tip="<?php printf( __( 'Extracts any enabled meta fields from the database and syncs them to %s.', 'wp-fusion' ), wp_fusion()->crm->name ); ?>"></span> | 

					<a href="<?php echo admin_url( 'user-edit.php' ); ?>?user_id=<?php echo $user->ID; ?>&wpf_profile_action=pull"><?php _e( 'Pull User Meta', 'wp-fusion' ); ?></a>

					<span class="dashicons dashicons-editor-help wpf-tip wpf-tip-bottom" data-tip="<?php printf( __( 'Loads any enabled meta fields from %s and saves them to the user record.', 'wp-fusion' ), wp_fusion()->crm->name ); ?>"></span> | 

					<a href="<?php echo admin_url( 'user-edit.php' ); ?>?user_id=<?php echo $user->ID; ?>&wpf_profile_action=show_meta"><?php _e( 'Show User Meta', 'wp-fusion' ); ?></a>

					<span class="dashicons dashicons-editor-help wpf-tip wpf-tip-bottom" data-tip="<?php _e( 'Displays all metadata found in the database for this user.', 'wp-fusion' ); ?>"></span> 
