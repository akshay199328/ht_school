	...
	public function show_logs_section() {
		...
		<div class="wrap">
			...
			<form method="get" id="mainform">
				...
				<h1 class="wp-heading-inline"><?php _e( 'WP Fusion Activity Log', 'wp-fusion' ); ?></h1>
				<a style="vertical-align: middle;" href="<?php echo admin_url( 'tools.php?page=wpf-settings-logs&flush-logs=true' ) ?>" name="flush-logs" class="button page-title-action"><?php _e( 'Flush all logs', 'wp-fusion' ); ?></a>
				...
				<span class="description" style="display: inline-block; padding: 5px 0;">
					<?php printf( __( 'For more information on the logs, %1$ssee our documentation%2$s.', 'wp-fusion' ), '<a href="https://wpfusion.com/documentation/getting-started/activity-logs/" target="_blank">', '</a>' ); ?>
					<?php printf( __( 'To go back to the main settings page, %1$sclick here%2$s.', 'wp-fusion' ), '<a href="' . admin_url( 'options-general.php?page=wpf-settings' ) . '">', '</a>' ); ?>
				</span>

				<?php if ( wp_fusion()->settings->get( 'logging_errors_only' ) ) : ?>

					<div class="notice notice-warning">
						<p><?php _e( '<strong>Note:</strong> The logs are currently set to record <strong>Only Errors</strong>, from the Advanced tab in the WP Fusion settings. Informational and debugging messages are not being recorded.', 'wp-fusion' ); ?></p>
					</div>
