...
class WPF_Log_Table_List extends WP_List_Table {
	...
	public function level_dropdown() {
		...
		$levels = array(
			...
			array( 'value' => 'http',      'label' => __( 'HTTP',      'wp-fusion' ) )
		);
			...
			if ( ! empty( $context['meta_array'] ) ) {
				...
				foreach ( $context['meta_array'] as $key => $value ) {
					...
					if ( is_array( $value ) && isset( $value['original'] ) ) {
						...
						$text = sprintf( __( 'This value was modified by the wpf_format_field_value filter before being sent to %1$s, using field format %2$s.', 'wp-fusion' ), wp_fusion()->crm->name, '<strong>' . $value['type'] . '</strong>' );
	...
	protected function user_dropdown() {
		...
		_e( '<p>If you can read this then your site ran out of memory while building the dropdown of log users.</p><p>You can fix this by clicking <strong>Flush All Logs</strong> above, or by increasing your available memory.</p>', 'wp-fusion' );
		echo '</p>';

		if ( function_exists( 'ini_get' ) ) {
			$memory_limit = ini_get( 'memory_limit' );
			echo '<p>' . __( 'Current memory limit:', 'wp-fusion' ) . '<code>' . $memory_limit . '</code></p>';
		}
		echo '</p></div>';
	...
	protected function date_select() {
		...
		?>
			<label for="start-date" class="screen-reader-text"><?php _e( 'Start date', 'wp-fusion' ); ?></label>

			<?php if ( ! empty( $_GET['startdate'] ) ) : ?>
				<input placeholder="<?php _e( 'Start date', 'wp-fusion' ); ?>" name="startdate" id="start-date" type="date" value="<?php echo $_GET['startdate']; ?>" />
			<?php else : ?>
				<input placeholder="<?php _e( 'Start date', 'wp-fusion' ); ?>" name="startdate" id="start-date" type="text" onfocus="(this.type='date')" />
			<?php endif; ?>

			<label for="end-date" class="screen-reader-text"><?php _e( 'End date', 'wp-fusion' ); ?></label>

			<?php if ( ! empty( $_GET['enddate'] ) ) : ?>
				<input placeholder="<?php _e( 'End date', 'wp-fusion' ); ?>" name="enddate" id="end-date" type="date" value="<?php echo $_GET['enddate']; ?>" />
			<?php else : ?>
				<input placeholder="<?php _e( 'End date', 'wp-fusion' ); ?>" name="enddate" id="end-date" type="text" onfocus="(this.type='date')" />
			<?php endif; ?>
