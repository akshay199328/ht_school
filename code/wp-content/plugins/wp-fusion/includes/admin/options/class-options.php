	...
	private function save_options() {
		...
		if ( $this->reset_options ) {
			...
			foreach ( $options as $id => $value ) {
				...
				} elseif ( ! isset( $settings[ $id ] ) && isset( $this->post_data[ $id ] ) ) {
	...
	private function initialize_settings( $settings ) {
		...
		foreach ( $settings as $id => $setting ) {
			...
			if ( 'integrations' == $setting['section'] && ! isset( $this->sections[ $this->setup['slug'] ]['integrations'] ) ) {
				$this->sections[ $this->setup['slug'] ] = wp_fusion()->settings->insert_setting_after( 'contact-fields', $this->sections[ $this->setup['slug'] ], array( 'integrations' => __( 'Integrations', 'wp-fusion' ) ) );
			}
...
" id="<?php echo $section_slug; ?>">
			...
			<p class="submit"><input name="Submit" type="submit" class="button-primary" value="<?php _e( 'Save Changes', 'wp-fusion' ); ?>" /></p>
		</form>
	...
	private function show_field_select( $id, $field, $subfield_id = null ) {
		...
		echo '<select id="' . ( $subfield_id ? $subfield_id : $id ) . '" class="select4-search' . $field['class'] . '" name="' . $this->option_group . '[' . $id . ']' . ( $subfield_id ? '[' . $subfield_id . ']' : '' ) . '" ' . ( $field['disabled'] ? 'disabled="true"' : '' ) . ' ' . ( $field['placeholder'] ? 'data-placeholder="' . $field['placeholder'] . '"' : '' ) . ' ' . ( $field['allow_null'] == false ? 'data-allow-clear="false"' : '' ) . ' ' . ( ! empty( $unlock ) ? 'data-unlock="' . trim( $unlock ) . '"' : '' ) . '>';
