...
class WPF_API {
	...
	public function handle_webhooks() {
		...
		if ( isset( $_REQUEST['wpf_action'] ) && 'thrivecart' != $_REQUEST['wpf_action'] ) {
			...
			if ( ! isset( $_REQUEST['access_key'] ) || wp_fusion()->settings->get( 'access_key' ) != $key ) {
			...
			if ( 'update' == $action || 'update_tags' == $action ) {
				...
				if ( false == $post_data['user_id'] ) {
					...
					wpf_log( 'notice', 0, '<strong>' . $action . '</strong> webhook received but no matching user found for contact ID <strong>' . $post_data['contact_id'] . '</strong>', array( 'source' => 'api' ) );
			...
			$message = 'Received <strong>' . $action . '</strong> webhook for contact ID <strong>' . $post_data['contact_id'] . '</strong>';
			...
			if ( $status ) {
				...
				if ( $action == $status ) {
					...
					$error = '<strong>Doing it wrong:</strong> An <strong>' . $action . '</strong> webhook is already being processed for contact ID ' . $post_data['contact_id'] . '. Sending simulteanous webhooks can result in unexpected behavior, and so this duplicate request will be ignored.';

				} elseif ( 'add' == $status ) {

					$error = '<strong>Doing it wrong:</strong> An <strong>add</strong> webhook is already being processed for contact ID ' . $post_data['contact_id'] . '. It\'s not necessary to send an <strong>' . $action . '</strong> webhook in addition, and doing so can result in unexpected behavior. This duplicate request will be ignored.';

				} elseif ( 'update' == $status && 'update_tags' == $action ) {

					$error = '<strong>Doing it wrong:</strong> An <strong>update</strong> webhook is already being processed for contact ID ' . $post_data['contact_id'] . '. It\'s not necessary to send an <strong>update_tags</strong> webhook in addition, and doing so can result in unexpected behavior. This duplicate request will be ignored.';
	...
	public function update_user( $post_data ) {
		...
		if ( ! empty( $post_data['async'] ) ) {
			...
			wp_fusion()->batch->quick_add( 'wpf_batch_pull_users_meta', array( $post_data['user_id'] ) );
	...
	public function add_user( $post_data ) {
		...
		wp_die( '<h3>Success</h3>User imported with ID ' . $user_id . '. <br /><br />Loaded tags:<pre>' . print_r( $tags, true ) . '</pre><br />Loaded meta:<pre>' . print_r( $meta, true ) . '</pre>', 'Success', 200 );
	 ...
	 */
	public function delete_transient( $user_id, $method, $contact_id ) {
