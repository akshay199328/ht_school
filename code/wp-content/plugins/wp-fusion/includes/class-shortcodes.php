...
class WPF_Shortcodes {
	...
	public function shortcodes( $atts, $content = '' ) {
		...
		if ( false !== strpos( $atts['tag'], '“' ) || false !== strpos( $atts['not'], '“' ) ) {
			return '<pre>' . __( '<strong>Oops!</strong> Curly quotes were found in a shortcode parameter of the [wpf] shortcode. Curly quotes do not work with shortcode attributes.', 'wp-fusion' ) . '</pre>';
		}
	...
	public function shortcode_user_meta( $atts, $content = null ) {
		...
		if ( false !== strpos( $atts['field'], '“' ) || false !== strpos( $atts['format'], '“' ) ) {
			return '<pre>' . __( '<strong>Oops!</strong> Curly quotes were found in a shortcode parameter of the [user_meta] shortcode. Curly quotes do not work with shortcode attributes.', 'wp-fusion' ) . '</pre>';
		}
