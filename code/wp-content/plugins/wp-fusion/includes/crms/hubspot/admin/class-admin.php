...
class WPF_HubSpot_Admin {
	...
	public function register_connection_settings( $settings, $options ) {
		...
		if ( empty( $options['hubspot_token'] ) && ! isset( $_GET['code'] ) ) {
			...
			$new_settings['hubspot_header']['desc'] .= '<td><a class="button button-primary" href="' . $auth_url . '">' . __( 'Authorize with HubSpot', 'wp-fusion' ) . '</a><br />';
			$new_settings['hubspot_header']['desc'] .= '<span class="description">' . __( 'You\'ll be taken to HubSpot to authorize WP Fusion and generate access keys for this site.', 'wp-fusion' ) . '</td>';
			...
			$new_settings['hubspot_refresh_token'] = array(
				...
				'desc'        => '<a href="' . $auth_url . '">' . sprintf( __( 'Re-authorize with %s', 'wp-fusion' ), wp_fusion()->crm->name ) . '</a>',
			);
