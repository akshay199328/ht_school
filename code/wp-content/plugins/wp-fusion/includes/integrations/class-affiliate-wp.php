...
class WPF_AffiliateWP extends WPF_Integrations_Base {
	...
	public function update_affiliate( $affiliate_data ) {

		$affiliate = affwp_get_affiliate( $affiliate_data['affiliate_id'] );

		if ( empty( $affiliate_data['rate'] ) ) {
			$affiliate_data['rate'] = affiliate_wp()->settings->get( 'referral_rate', 20 );
		}
	...
	public function referral_accepted( $affiliate_id, $referral ) {
		...
		if ( 'woocommerce' == $referral->context ) {
			...
			add_filter(
				'wpf_gform_pre_submission', function( $update_data, $user_id, $contact_id, $form_id ) use ( &$referrer_data ) {
	...
	public function add_affiliate( $affiliate_id ) {
		...
		if ( 'active' == $affiliate->status ) {
			$approved_tags = wp_fusion()->settings->get( 'awp_apply_tags_approved', array() );
