...
class WPF_EDD extends WPF_Integrations_Base {
	...
	public function init() {
		...
		// Payment status
		add_action( 'edd_view_order_details_update_after', array( $this, 'order_details_sidebar' ) );
	...
	public function complete_purchase( $payment_id, $doing_async = false, $force = false ) {
		...
		// We don't need to sync the meta data during a renewal order
		$send_meta = true;

		if ( 'edd_subscription' == get_post_status( $payment_id ) && ! wp_fusion()->crm_base->is_field_active( 'order_date' ) ) {
			$send_meta = false;
		}
		...
		if ( (int) $user_id < 1 ) {
			...
			} elseif ( true == $send_meta ) {

				// If contact is found for user, update their info (but not during a renewal payment)
	...
	public function order_details_sidebar( $payment_id ) {
		...
		<div id="edd-wpf-status" class="postbox edd-wpf-status">
			...
			<div class="edd-admin-box">
				...
				<?php $contact_id = $payment->get_meta( wp_fusion()->crm->slug . '_contact_id' ); ?>
				...
				<div class="edd-order-wpf-status edd-admin-box-inside">
					...
					<p>
						<a href="<?php echo esc_url( add_query_arg( array( 'edd-action' => 'wpf_process', 'purchase_id' => $payment_id ) ) ); ?>" class="button-secondary"><?php _e( 'Process WP Fusion actions again ', 'wp-fusion' ); ?></a>
					</p>
