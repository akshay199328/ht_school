	...
	public function run( $record, $ajax_handler ) {
		...
		foreach ( $form_settings['wpf_fields_map'] as $field ) {
			...
			if ( ! empty( $sent_data[ $field['local_id'] ] ) ) {
				...
				if ( is_array( $sent_data[ $field['local_id'] ] ) ) {
					$type = 'checkboxes';
				} elseif ( '1' == $sent_data[ $field['local_id'] ] || 'true' == $sent_data[ $field['local_id'] ] ) {
					$type = 'checkbox'; // boolean true
				} elseif ( '0' == $sent_data[ $field['local_id'] ] || 'false' == $sent_data[ $field['local_id'] ] ) {
					$type = 'checkbox'; // boolean false
				} else {
					$type = 'text';
				}

				$update_data[ $field['remote_id'] ] = apply_filters( 'wpf_format_field_value', $sent_data[ $field['local_id'] ], $type, $field['remote_id'] );

				if ( false == $email_address && is_email( $sent_data[ $field['local_id'] ] ) ) {
