...
class WPF_Forms_Helper {
	...
	public static function process_form_data( $args ) {
		...
		if ( wp_fusion()->settings->get( 'auto_login_forms', false ) == true && ! wpf_is_user_logged_in() ) {

			wpf_log( 'info', 0, 'Starting auto-login session from form submission for contact ID ' . $contact_id, array( 'source' => sanitize_title( $integration_name ) ) );
