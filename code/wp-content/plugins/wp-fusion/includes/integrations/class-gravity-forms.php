...
class WPF_GForms_Integration extends GFFeedAddOn {
	...
	public function init() {
		...
		// For some reason this changed in GForms 2.5 beta.
		// @todo update this to version_compare() once 2.5 comes out of beta

		if ( strpos( GFCommon::$version, '2.5-beta' ) !== false ) {
			$this->setting_key = '_gform_setting';
		} else {
	...
	public function settings_wpf_lists( $field ) {
		...
		echo '<select multiple="" class="select4 select4-hidden-accessible" name="' . $this->setting_key . '_wpf_lists[]" data-placeholder="Select lists" tabindex="-1" aria-hidden="true">';
