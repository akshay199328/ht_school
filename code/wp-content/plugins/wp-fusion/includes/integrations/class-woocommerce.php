...
class WPF_Woocommerce extends WPF_Integrations_Base {
	...
	public function register_settings( $settings, $options ) {
		...
		$settings['email_optin'] = array(
			...
			'type'    => 'checkbox',
			'section' => 'integrations',
			'unlock'  => array( 'email_optin_message', 'email_optin_default', 'wpf_options-email_optin_tags', 'email_optin_tags' ),
		);

		$settings['email_optin_message'] = array(
			'title'   => __( 'Email Optin Message', 'wp-fusion' ),
			'std'     => __( 'I consent to receive marketing emails', 'wp-fusion' ),
			'type'    => 'text',
			'format'  => 'html',
			'section' => 'integrations',
		);
		...
		$settings['email_optin_tags'] = array(
			...
			'desc'    => __( 'Apply these tags when the email optin box is checked.', 'wp-fusion' ),
			'std'     => array(),
			'type'    => 'assign_tags',
			'section' => 'integrations',
		);
	 ...
	 * Order has contact.
	 *
	 * Check whether a contact record has been created yet for a given order.
	 *
	 * @since  3.36.1
	 *
	 * @param  WC_Order $order  The order.
	 * @return bool     Has contact
	 */
	public function order_has_contact( $order ) {

		// If the order is for a registered user we'll check their record first.

		$user_id = $order->get_user_id();

		if ( $user_id && wpf_get_contact_id( $user_id ) ) {
			return true;
		}

		// If it's a guest checkout there will be a contact ID saved to the order.

		if ( $order->get_meta( wp_fusion()->crm->slug . '_contact_id', true ) ) {
			return true;
		}
	...
	public function enqueue_async_checkout_script() {
		...
		if ( is_checkout() && true == wp_fusion()->settings->get( 'woo_async_new' ) ) {
	...
	public function process_order( $order_id ) {
				...
				foreach ( $product_taxonomies as $product_taxonomy ) {
					...
					if ( ! empty( $product_terms ) ) {
						...
						foreach ( $product_terms as $term ) {
							...
							if ( 'product_cat' == $product_taxonomy && wp_fusion()->settings->get( 'woo_category_tagging' ) == true ) {
		...
		// Optin tags
		if ( ! empty( $order_data['email_optin'] ) && ! empty( wp_fusion()->settings->get( 'email_optin_tags' ) ) ) {
			$apply_tags = array_merge( $apply_tags, wp_fusion()->settings->get( 'email_optin_tags' ) );
		}
		...
		if ( ! empty( $remove_tags ) ) {
			...
			if ( empty( $user_id ) ) {
				...
				wpf_log( 'info', 0, 'Removing transaction failed tags, guest checkout for contact ID ' . $contact_id . ': ', array( 'tag_array' => $remove_tags ) );
	 ...
	 *
	 * @param  int  $order_id The order ID.
	 * @param  bool $force    Whether or not to force-bypass the async process.
	 * @return bool  Whether or not the order was successfully processed.
	 */

	public function woocommerce_apply_tags_checkout( $order_id, $force = false ) {
		...
		// There are some scenarios where we want to process the order right away, regardless of the async setting....
		...
		if ( wp_doing_ajax() && true == wp_fusion()->settings->get( 'woo_async' ) ) {

			$order = wc_get_order( $order_id );

			if ( true == wp_fusion()->settings->get( 'woo_async_new' ) ) {
	...
	public function order_status_changed( $order_id, $old_status, $new_status, $order ) {
		...
		$apply_tags = wp_fusion()->settings->get( 'woo_status_tagging_wc-' . $new_status, array() );
		...
		if ( ! $this->order_has_contact( $order ) ) {
			$contact_id = $this->create_update_customer( $order );
		}
	...
	public function new_order( $order_id, $order = false ) {
		...
		if ( 'pending' == $order->get_status() && ! did_action( 'woocommerce_order_status_changed' ) ) {
	...
	public function add_to_cart_apply_coupon() {
		...
		foreach ( $coupons as $coupon_id ) {
			...
			$should_apply = apply_filters( 'wpf_auto_apply_coupon_for_user', $should_apply, $coupon_id, wpf_get_current_user_id() );
	...
	public function email_optin_checkbox() {
		...
		if ( true == wp_fusion()->settings->get( 'email_optin' ) ) {

			if ( 'unchecked' == wp_fusion()->settings->get( 'email_optin_default' ) ) {
				$default = false;
			} else {
				$default = true;
			}
			...
			woocommerce_form_field(
				'email_optin', array(
					...
					'label'       => wp_fusion()->settings->get( 'email_optin_message' ),
				), $default
			);
