...
class WPF_YITH_Vendors extends WPF_Integrations_Base {
	...
	public function pre_user_register( $username ) {
		...
		if ( isset( $_POST['vendor-antispam'] ) && true == wp_fusion()->settings->get( 'yith_vendors_defer' ) ) {
	...
	public function account_approved( $owner_id ) {
		...
		if ( true == wp_fusion()->settings->get( 'yith_vendors_defer' ) ) {
			wp_fusion()->user->user_register( $owner_id );
		}

		$apply_tags = wp_fusion()->settings->get( 'yith_vendors_approved_tags' );
