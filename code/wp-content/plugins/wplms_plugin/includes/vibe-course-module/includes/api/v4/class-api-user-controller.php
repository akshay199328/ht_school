			...
			$curriculum = bp_course_get_curriculum($course_id,$this->user->id); 
			 if(empty($curriculum)){
				...
				if(!empty($package)){
					...
					$return = array('status'=>1,'package' => $pack,'package_type' =>$package['package_type']);
				}else{
					...
					if(is_numeric($item)){
						...
						if(bp_course_get_post_type($item) == 'unit'){
							...
							if(strtolower($course_type) == "live classes"){
								$get_live_course_start_data = $wpdb->get_results("SELECT bm.course_id,bm.batch_name,bm.variation_id,bsm.user_id,bm.start_on,bm.meeting_id,bm.meeting_passcode FROM ht_batch_master AS bm LEFT JOIN ht_batch_student_mapping AS bsm ON bsm.batch_id = bm.id WHERE bsm.user_id = '".$user_id."' AND course_id = '".$course_id."'");
							...
							if(function_exists('bp_course_fetch_user_quiz_retake_count') && bp_is_active('activity')){
								...
								$retake_count = bp_course_fetch_user_quiz_retake_count($item,$user_id);
				...
				$sql = $wpdb->get_results("SELECT sum(creds) as total_creds FROM $my_cred_table WHERE user_id = '".$user_id."' and ref !='signup_referral' and ref !='mycred_default' ");
