 ...
 */
  ...
  function bp_wplms_get_quiz_data($user_id,$item_id,$item=null,$course=null,$status=null,$activity_id=null){
    //Get all questions.
    if(!empty($user_id)){
      if(empty($status)){
    ...
    if($quiz_access_flag){
      ...
      $sql = $wpdb->get_results("SELECT SUM(creds) as total_creds FROM $my_cred_table WHERE ref='".$event_quiz_type."_points' AND user_id = '".$user_id."' and ref_id = '".$item_id."' ");
