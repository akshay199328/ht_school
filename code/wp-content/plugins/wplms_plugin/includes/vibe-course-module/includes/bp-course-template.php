...
Class BP_Course_Template{
	...
	function get_course_meta($course_id){
		...
		$this->course_meta[$course_id] = apply_filters('wplms_course_meta',$meta,$course_id);
		return $this->course_meta[$course_id];
	}
	...
	*/
	...
	function get_course_credits($args){
		...
		if($type == 'array' && isset($this->course_credits_Array[$id]) ){
			return $this->course_credits_Array[$id];

		}
		if($type == 'html' && isset($this->course_credits[$id]))
		...
		$this->course_credits_Array[$id] = $credits;
		if($type == 'array'){
			return $credits;
		}
		if(count($credits) > 1 ){
			if(is_wplms_4_0() && function_exists('vibe_get_option') && vibe_get_option('offload_scripts')){
				$credits_html .= '<div class="pricing_course">';
				foreach($credits as $key => $credit){
					$credits_html .= '<a href="'.$key.'" class="course_price_option amount" style="padding:5px;">'.$credit.'</a>';
				}
				$credits_html .= '</div>';
			}else{
				$credits_html .= '<div class="pricing_course">
	...
	function bp_course_display_rating($reviews){
	  ...
	  $meta = '<strong class="course-star-rating">
	    ...
	    $reviews  = floatval($reviews);
      	if($reviews >= 0){
	        $width = ($reviews/5)*100;
	        $meta .='<small class="bp_blank_stars" style="line-height:2;"><small style="width:'.$width.'%;" class="bp_filled_stars"></small></small>';
      	}
	  $meta .='</strong>';    
...
function bp_course_get_quiz_questions($quiz_id,$user_id){
	...
	if(empty($questions) || (isset($questions['ques']) && empty($questions['ques'])) ){
	    ...
	    if(empty($questions) || !is_array($questions)){ // Fallback for Older versions
	      	$raw_questions = get_post_meta($quiz_id,'vibe_quiz_questions',true);
	      	$raw_questions=maybe_unserialize($raw_questions);
	      	
	      	if(empty($raw_questions['ques'])){
	      		$questions = array('ques'=>array(),'marks'=>array());
	      		if(is_array($raw_questions)){
		      		foreach($raw_questions as $question){
		      			$questions['ques'][]=$question['data']['id'];
...
function bp_course_get_question_details($question_id,$force = null){
	...
	if( has_shortcode( $content, 'match' ) ) {
		...
		if(!empty($content_array) && !empty($content_array)){
			...
			preg_match_all("/<li\b[^>]*>(.*?)<\/li>/", $content_array[0][2], $s_items);
			if(!empty($s_items)){
		...
		preg_match_all( '/' . get_shortcode_regex(array('select')) . '/', $content, $matches, PREG_SET_ORDER );
		$options_arr = [];
		if ( !empty( $matches ) ){
			foreach ( $matches as $shortcode ) {
				preg_match('/options=[\'|"](.*?)[\'|"]/',$shortcode[3],$options);

				if(!empty($options[1])){
					$options_arr[] = explode(',', $options[1]);
				}else{
					$options_arr[] = [];
				}
			}
			$question['options_arr'] = $options_arr;
		}
...
function bp_course_remove_user_quiz_status($user_id,$quiz_id){
	...
	if(function_exists('bp_is_active') && bp_is_active('activity')){
		...
		if(is_numeric($activity_id)){
			...
			bp_course_change_activity_meta_key_name( $activity_id ,'old_');
			//update the meta  
		}
	}
}

function bp_course_change_activity_meta_key_name($activity_id,$prefix){
	global $bp,$wpdb;

	$results = $wpdb->get_results($wpdb->prepare("UPDATE {$bp->activity->table_name_meta} SET meta_key = CONCAT(%s,meta_key) WHERE activity_id=%d",$prefix,$activity_id));
...
function bp_course_generate_user_result($quiz_id,$user_id,$user_result,$activity_id){
	...
	if(!is_numeric($activity_id)){
		...
		$custom = apply_filters('wplms_custom_bp_course_generate_user_result',true,$quiz_id,$user_id,$user_result,$activity_id);
...
if(!function_exists('bp_course_get_curriculum')){
	function bp_course_get_curriculum($course_id = NULL,$user_id=NULL){
		...
		$course_curriculum = apply_filters('bp_course_get_course_curriculum',$course_curriculum,$course_id,$user_id);
...
function bp_course_get_curriculum_units($course_id=NULL,$user_id=NULL){
	$units=array();
	if(!isset($course_id) || !is_numeric($course_id))
		return $units;
...
function bp_course_get_curriculum_quizes($course_id = NULL,$user_id=NULL){
	$quizes=array();
	if(!isset($course_id) || !is_numeric($course_id))
		return $quizes;
  	...
  	if(function_exists('bp_is_active') && bp_is_active('activity')){
  		if(!empty($status) && in_array($status,array('active','expired'))){
			$query = $wpdb->get_results(apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("
  			...
  			$query = $wpdb->get_results(apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("
	  		SELECT DISTINCT item_id as id,type as status
...
function bp_get_course_check_course_complete($args=NULL){ // AUTO EVALUATION FOR COURSE
	...
	if($stop_finish_course_check){
		if(is_Array($stop_finish_course_check)){
			return $stop_finish_course_check;
		}
		return;
	}
	...
	if(isset($course_curriculum) && count($course_curriculum)){
		...
		foreach($course_curriculum as $unit_id){
			...
			if(empty($unittaken) && in_array(bp_course_get_post_type($unit_id),array('quiz','wplms-assignment'))){
				$unittaken=get_user_meta($user_id,$unit_id,true);
				if($unittaken && bp_course_get_post_type($unit_id)=='quiz'){
		...
		$flag = apply_filters('wplms_finish_course_check',$flag,$course_curriculum,$user_id);
		if(!$flag){
        ...
        if($unitkey == 0){ // Start of Course
            ...
            if(!isset($pre_unit_time) || $pre_unit_time ==''){
                ...
                if(!empty($pre_unit_time)){
                      ...
                      $value = apply_filters('wplms_drip_value',$value,$units[$pre_unit_key],$course_id,$units[$unitkey],$units,$user_id);
...
function bp_course_get_course_retakes($course_id,$user_id=null){
	$retakes = apply_filters('wplms_course_retake_count',get_post_meta($course_id,'vibe_course_retakes',true),$course_id,$user_id);

	return $retakes;
}

function bp_course_get_user_course_retakes($course_id,$user_id){

	global $wpdb,$bp;
    $table_name = $bp->activity->table_name;
    $course_retakes = $wpdb->get_var($wpdb->prepare( "
                        SELECT COUNT(*) FROM {$table_name} AS activity
    ...
    return is_numeric($course_retakes)?$course_retakes:0;
}
...
function bp_course_get_quiz_results_meta($quiz_id,$user_id,$activity_id = NULL,$old=null){
	global $wpdb,$bp;
	$result=0;
	if($old){
		$meta_key = 'old_quiz_results';
	}else{
	...
	if(bp_is_active('activity')){
		if(!empty($activity_id)){
			...
			$result = $wpdb->get_var($wpdb->prepare("
				...
				AND a.secondary_item_id = %d
				AND a.user_id = %d
				ORDER BY a.id DESC LIMIT 0,1",$meta_key ,$quiz_id ,$user_id));
			...
			if(!empty($question['content'])){
				...
				echo '<li>
					...
					if($question['type'] == 'survey'){
						...
						$show_correct_answer_explanation = apply_filters('wplms_show_correct_answer_explanation_unattempted',(isset($question['max_marks']) && $question['max_marks'] !=''),$question_id,$quiz_id);
