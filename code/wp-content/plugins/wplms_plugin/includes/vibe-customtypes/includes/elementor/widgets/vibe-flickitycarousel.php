...
class Wplms_Vibe_Carousel extends \Elementor\Widget_Base  // We'll use this just to avoid function name conflicts 
{
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'post_type',
			[
				'label' => __('Select Post Type', 'wplms'),
		...
		$this->add_control(
			'taxonomy',
			[
				'label' => __('Taxonomy', 'wplms' ),
				'type' => \Elementor\Controls_Manager::TEXT,
				'input_type' => 'text',
				'placeholder' => __( 'course-cat,event-type..', 'wplms' ),
			]
		);
		...
		$this->add_control(
			'term',
			[
				'label' => __('Enter Taxonomy Term Slugs (comma separated)', 'wplms'),
				'type' => \Elementor\Controls_Manager::TEXT,
				'input_type' => 'text',
		...
		$this->add_control(
			...
			[
				...
				'default' => 'recent',
				'options' => array(
	                'recent' => _x('Recently published','elementor','wplms'),
	                'popular' => _x('Most Students','elementor','wplms'),
	                'featured' => _x('Featured','elementor','wplms'),
	                'rated'  => _x('Highest Rated','elementor','wplms'),
	                'reviews' => _x('Most Reviews','elementor','wplms'),
	                'start_date' => _x('Upcoming Courses (Start Date)','elementor','wplms'),
	                'expired_start_date'=>_x('Expired Courses (Past Start Date)','elementor','wplms'),
	                'free'=> _x('Free Courses','elementor','wplms'),
	                'random' => _x('Random','elementor','wplms'),
                ),
			]
		);
		...
		$this->add_control(
			'carousel_space',
			[
				'label' =>__('Space between carousel blocks', 'wplms'),
				'type' => \Elementor\Controls_Manager::NUMBER,
				'min' => 1,
				'max' => 100,
				'step' => 1,
		...
		do_action('wplms_vibe_carousel',$this);
	...
	protected function render() {
	    ...
	    course_type="'.(isset($settings['course_type'])?$settings['course_type']:'').'"
