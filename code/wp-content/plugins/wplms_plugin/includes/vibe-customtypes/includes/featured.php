...
function thumbnail_generator($custom_post,$featured_style,$cols='medium',$n=100,$link=0,$zoom=0){
    ...
    switch($featured_style){
            ...
            case 'course_card':
                ...
                $init->featured_id = $post->ID;//Required as elementor 
                
                $layout = new WP_Query(apply_filters('wplms_course_card_query',array(
            ...
            case 'course':
                    ...
                    if(get_post_type($custom_post->ID) == 'course'){
                        ...
                        if(isset($st) && $st !='')
                            $meta .= apply_filters('vibe_thumb_student_count','<strong><i class="vicon vicon-user"></i> '.$st.'</strong>');
           ...
           case 'course2':
                    ...
                    if(get_post_type($custom_post->ID) == 'course'){
                        ...
                        $meta =  apply_filters('vibe_thumb_rating',$meta,$featured_style,$rating);
                        $meta .= apply_filters('vibe_thumb_reviews','(&nbsp;'.(isset($rating_count)?$rating_count:'0').'&nbsp;'.__('REVIEWS','wplms').'&nbsp;)',$featured_style).'</div>';
                        $free_course = get_post_meta($custom_post->ID,'vibe_course_free',true);
                            ...
                            if(isset($coauthors)){
                                foreach($coauthors as $k=>$inst){
                                    ...
                                    $instructor = '<a href="'.$author_domain.'" 
                                        title="'.sprintf(__('Course Author %s','wplms'),$displayname).'">'.
            ...
            case 'course4':
                    ...
                    if($enable_instructor){
                        if ( function_exists('get_coauthors')) {
                            ...
                            if(isset($coauthors)){
                                foreach($coauthors as $k=>$inst){
                                    ...
                                    $author_domain = bp_core_get_user_domain( $instructor_id );
                                    $instructor = '<a href="'.$author_domain.'" class="course_instructor" 
                                        title="'.__('Course Author','wplms').'" style="float:none;padding:2px;">'.$displayname.'</a>';
                                    $instructors[] = $instructor;
                                }
                            }
                        }
                        ...
                        if(!empty($instructors)){
                            ...
                            $displayname = bp_core_get_user_displayname( $instructor_id );
                            $instructor = '<a href="'.$author_domain.'" class="course_instructor" 
                                title="'.__('Course Author','wplms').'">'.$displayname.'</a>';
                        }
                        $return .= $instructor;
                    }
    ...
    //                $return .= '<h4 class="block_title"><a href="'.get_permalink($custom_post->ID).'" title="'.$custom_post->post_title.'">'.$custom_post->post_title.'</a></h4>';
            ...
            case 'course5':
                    ...
                    if(function_exists('bp_course_get_start_date')){
                        ...
                        $return .='<span><span class="vicon vicon-calendar"><span><span>'.(date_i18n( get_option( 'date_format' ), strtotime( $date ))).'</span></span>';
                    }
                    $return .= '<div class="course_price">'.bp_course_get_course_credits(array('id' => $custom_post->ID)).'</div>';
            ...
            case 'course6':
                    ...
                    if(function_exists('bp_course_get_start_date') && $custom_post->post_type == 'course'){
                        ...
                        if(!empty($date) && strtotime( $date ) > time()){
                            ...
                            $date = str_replace('-','/',$date);
                            $return .='<span class="course_start_date"><span class="vicon vicon-calendar"></span><span>'.(date_i18n( get_option( 'date_format' ), strtotime( $date ))).'</span></span>';
                        }
                    } 
                    ...
                    $st = get_post_meta($custom_post->ID,'vibe_students',true);
                    if(isset($st) && $st !='')
                         $return .= apply_filters('vibe_thumb_student_count','<strong><i class="vicon vicon-user"></i> '.$st.'</strong>');
            ...
            case 'course7' :
                $return ='';
                $return .= '<div class="block course7">';
                $return .= '<div class="block_media">';
                $return .= '<a href="'.get_permalink($custom_post->ID).'">'.get_the_post_thumbnail($custom_post->ID,'medium').'</a>';
                $return .= '</div>';
                $return .= '<div class="block_content">';
                $return .= '<h4 class="block_title"><a href="'.get_permalink($custom_post->ID).'" title="'.$custom_post->post_title.'">'.$custom_post->post_title.'</a></h4>';
                $return .= '<p class="block_description">'.get_the_excerpt($custom_post->ID).'</p>';
                ob_start();
                the_course_button($custom_post->ID);
                $return .= ob_get_clean();
                if(is_user_logged_in()){
                 $user_id = get_current_user_id();
                 if(bp_course_is_member($custom_post->ID,$user_id)){
                    $progress = bp_course_get_user_progress($user_id,$custom_post->ID);
                    if(empty($progress)){
                       $progress = 0;
                    }
                    $return .= do_shortcode('[progressbar percentage="'.$progress.'" bg="#eee" bar_color="42a1f4"]');
                 }else{
                    $return .= bp_course_get_course_credits(array('id' => $custom_post->ID));
                 }
                }else{
                 $return .= bp_course_get_course_credits(array('id' => $custom_post->ID)); 
                }
            ...
            case 'course8' :
                ...
                $return .='<div class="block courseitem course8"  data-id="'.$custom_post->ID.'">';
                $return .='<div class="block_media">';

                $featured = get_post_meta($custom_post->ID,'featured',true);
                if(!empty($featured)) {
                    $return .= '<span class = "featured_btn"><i class="vicon vicon-sar"></i><small>'._x('Featured','','wplms').'</small></span>';  
                }
               
                $return .= apply_filters('vibe_thumb_featured_image',featured_component($custom_post->ID,$cols),$featured_style);
                 $course_duration = get_the_course_time($custom_post->ID); 
                if(!empty($course_duration)) {
                    $return .= '<div class = "course_time"><i class="icon-clock"></i>  '.$course_duration.'</div>';  
                }
                ...
                if(!empty($cats)){
                    foreach($cats as $cat){
                        $return .= '<a href="'.get_category_link($cat->term_id ).'" class="postblock_cat cat_btn">'.$cat->name.'</a>';
                    }
                }
                $return .= apply_filters('vibe_thumb_desc','<p class="block_desc">'.vibe_custom_types_excerpt($n,$custom_post->ID).'</p>',$featured_style);
                $return .= '<div class="more">';
                $return .= '<a href="'.get_permalink($custom_post->ID).'" class="more_btn">'._x('Course Info','','wplms').'</a></div>';

                $return .= '<div class="course_meta">';
                $reviews = get_post_meta($custom_post->ID,'average_rating',true);
                
                $return .='<p class="star-rating">';
                if(function_exists('bp_course_display_rating')){
                   $return .= bp_course_display_rating($reviews);
                }else{
                    for($i=1;$i<=5;$i++){
                ...
                $return .= '<p class="course_price">'.bp_course_get_course_credits(array('id' => $custom_post->ID)).'</p></div>';
                $return .=apply_filters('wplms_course_thumb_extras','');
                $return .= '</div></div>';
            ...
            case 'course9' :
               ...
               $return ='';
                $return .= '<div class="block course9">';
                $return .= '<div class="block_media">';
                $return .= '<a href="'.get_permalink($custom_post->ID).'">'.get_the_post_thumbnail($custom_post->ID,'medium').'</a>';
                $return .= '</div>';
                $return .= '<div class="block_content">';
                $return .= '<div class="block_div_course9">';
                $return .= '<h4 class="block_title"><a href="'.get_permalink($custom_post->ID).'" title="'.$custom_post->post_title.'">'.$custom_post->post_title.'</a></h4>';
                $return .= '<p class="block_desc">'.get_the_excerpt($custom_post->ID).'</p>';
                $return .= '</div>';
                $return .= '<div class="bottom_desc">';
                $return .= '<div class="course_block_bottom">';
                $return .= '<a href="https://wplms.io/demos/demo14/members/wplms_admin_de/" class="course_instructor" title="Course Author">wplms_admin_de</a>';
                $enable_instructor = apply_filters('wplms_display_instructor',true,$custom_post->ID);
                if($enable_instructor){
                    if ( function_exists('get_coauthors')) {
                        $coauthors = get_coauthors( $custom_post->ID );
                    ...
                    if(!empty($instructors)){
                        ...
                        $displayname = bp_core_get_user_displayname( $instructor_id );
                        $instructor = '<a href="'.$author_domain.'" class="course_instructor" 
                                    title="'.sprintf(__('Course Author %s','wplms'),$displayname).'"></a>';
                    }
                    $return .= $instructor;
                }
                $st = get_post_meta($custom_post->ID,'vibe_students',true);
                ...
                $meta =  apply_filters('vibe_thumb_rating',$meta,$featured_style,$rating);
                $meta .= '</div>';
                $return .='<i class="hide">4.3</i>
                   <small class="bp_blank_stars"><small style="width:86%;" class="bp_filled_stars"></small></small>
                </strong>';
                $return .='</div>';
                $return .= '<div class="course_price">'.bp_course_get_course_credits(array('id' => $custom_post->ID)).'</div>';
                $return .= '</div>';
            ...
            case 'course10' :
                ...
                $return .= '<div class="block course10">';
                $return .= '<div class="block_media">';
                $return .= '<a href="'.get_permalink($custom_post->ID).'">'.get_the_post_thumbnail($custom_post->ID,'medium').'</a>';
                $return .= '</div>';
                $return .= '<div class="block_content">';
           ...
           case 'side':
                    ...
                    $return .='<div class="block_content">';
                    $return .= apply_filters('vibe_thumb_heading','<h4 class="block_title"><a href="'.get_permalink($custom_post->ID).'" title="'.$custom_post->post_title.'">'.$custom_post->post_title.'</a></h4>',$featured_style);
                    $return .= apply_filters('vibe_thumb_date','<div class="date"><small>'. get_the_time('F d,Y').''.((strlen($category)>2)? ' / '.$category:'').' / '.get_comments_number( '0', '1', '%' ).' '.__(' Comments','wplms').'</small></div>',$featured_style);
                    $return .= apply_filters('vibe_thumb_desc','<p class="block_desc">'.vibe_custom_types_excerpt($n,$custom_post->ID).'</p>',$featured_style);
                    $return .='</div>';
                    $return .='</div>';
                break;    
            ...
            case 'testimonial': 
             ...
             case 'blogpost':
                    ...
                    if($custom_post->post_type == 'article'){
                        $category = get_the_term_list( $custom_post->ID, 'article_type', '', ' / ' );
                    }
                    ...
                    $return .= '<div class="blog-item">
                                ...
                                </div>',$featured_style).'<div class="blog-item-content">
            ...
            case 'event_card':
                ...
                $return .= ' <span class="event_icon" style="color:'.$color.'"><i class="'.$icon_class.'"></i></span>
                        <h4 style="background:'.$color.'"><i class="'.$icon_class.'"></i> '.__('Event ','wplms').'</label><span><a href="'.get_permalink($custom_post->ID).'">'.get_the_title($custom_post->ID).'</a></span></h4>
                        <ul>
                        ';
                        
                        if(isset($start_date) && $start_date !=''){
                           $return .= '<li><label><i class="icon-calendar"></i> '.__('Start Date ','wplms').'</label><span>'.date('F j Y',strtotime($start_date)).'</span></li>';
                        } 
                        if(isset($end_date) && $end_date !=''){
                            $return .= '<li><label><i class="icon-calendar"></i> '.__('End Date ','wplms').'</label><span>'.date('F j Y',strtotime($end_date)).'</span>';
                        }
                        if(isset($start_time) && $start_time !=''){
                             
                            $return .= '<li><label><i class="icon-clock"></i> '.__('Start Time ','wplms').'</label><span>'.$start_time.'</span>';
                        } 
                        if(isset($end_time) && $end_time !=''){
                            $return .= '<li><label><i class="icon-clock"></i> '.__('End Time ','wplms').'</label><span>'.$end_time.'</span>';
                        }
                        if(vibe_validate($all_day)){
                            $return .= '<li><label><i class="icon-circle-full"></i> '.__('All Day ','wplms').'</label><span>'.__('Yes','wplms').'</span>';
                        }
                        if(vibe_validate($repeatable)){
                            $return .= '<li><label><i class="icon-flash"></i> '.__('Frequency ','wplms').'</label><span>'.__('Every ','wplms').((isset($repeat_value) && $repeat_value > 1)?$repeat_value:'').' '.$repeat_unit.' '.__('for ','wplms').$repeat_count.' '.$repeat_unit.'</span>';
                        }
                        if(vibe_validate($show_location)){
                            $return .= '<li><label><i class="icon-pin-alt"></i> '.__('Venue ','wplms').'</label><span>'.(isset($location['staddress'])?$location['staddress']:'').(isset($location['city'])?', '.$location['city']:'').(isset($location['state'])?', '.$location['state']:'').(isset($location['country'])?', '.$location['country']:'').(isset($location['pincode'])?' - '.$location['pincode']:'').'</span>';
                        }
                        $return .= '</ul>
                        <a href="'.get_permalink($custom_post->ID).'" class="event_full_details tip" title="'.__('View full details','wplms').'" style="background:'.$color.'"><i class="icon-plus-1"></i></a>
                    </div>';
            ...
            case 'generic_card':
                $return .='<div class="block generic_card">';
                $return .='<div class="block_media">';
            ...
            break; 
            case 'blog_card':
                $return .='<div class="block blog_card">';
                $return .='<div class="block_media">';
...
function featured_component($custom_post_id,$cols='',$style=''){
    ...
    if(has_post_thumbnail($custom_post_id)){
        $custom_post_thumbnail=  '<a href="'.get_permalink($custom_post_id).'">'.get_the_post_thumbnail($custom_post_id,$cols).'</a>';
...
if(!function_exists('vibe_member_block')){
    function vibe_member_block($user,$style,$field_names = null,$width='150',$link=0){
        ...
        switch($style){
            case 'member2':
               ...
               if(!empty($field_names)){
                    foreach($field_names as $field){
                        if(bp_get_profile_field_data('field='.$field.'&user_id='.$user->ID)) {
                            $member_html .= '<span>'.bp_get_profile_field_data('field='.$field.'&user_id='.$user->ID).'</span><br/>';
                        }
                    }
               }
                                ...
                                if ( !empty( $g->fields ) ) {
                                    foreach ( $g->fields as $field ) {
                                        ...
                                        if(!empty($url) && strlen($url)>2){
                                            ...
                                            $member_html .= '<li><a href="'.$url.'" class="'.$n.'"><i class="vicon vicon-'.$n.'"></i></a></li>';
                                        }
                                    }
                                }
                            }
            ...
            case 'member5':
                $link = bp_core_get_user_domain($user->ID);
                $member_html .='<div class="member_block5">';
                $member_html .='<div class="member_avatar">'.(empty($link)?'':'<a href="'.$link.'">').$avatar.(empty($link)?'':'</a>');
               $member_html .='</div>';
                $member_html .='<div class="member_info">';
                $member_html .='<h3><a href="'.$link.'">'.$user->display_name.'</a></h3>';
