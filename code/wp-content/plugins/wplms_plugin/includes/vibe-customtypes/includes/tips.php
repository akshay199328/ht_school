...
class WPLMS_tips{
	...
	private function __construct(){
		...
		if(is_array($this->lms_settings) && isset($this->lms_settings['general'])){
			...
			foreach($this->settings as $key=>$setting){
				switch($key){
					case 'wplms_commissions_migrate_to_activity_revert':
					if(!empty($this->settings['wplms_commissions_migrate_to_activity_revert'])){
						add_filter('wplms_commissions_migrate_to_activity',array($this,'wplms_commissions_migrate_to_activity'));
					}
					case 'guest_user_id':
					if(!empty($this->settings['guest_user_id'])){
						...
						if($user != false){
							...
							add_filter('wplms_take_course_button_html',array($this,'wplms_guest_popup_filter'));
							add_filter('wplms_course_non_loggedin_user',array($this,'wplms_guest_popup_filter'));
							//add_filter('wplms_private_course_button_html',array($this,'wplms_guest_popup_filter'));
							...
							//check if one session per user is enabled 
							if(!empty($this->settings) && !empty($this->settings['one_session_per_user'])){
								remove_filter( 'authenticate',array($this,'one_session_per_user'), 30, 3 );
								add_filter( 'authenticate',array($this,'one_session_per_user_check_guest'), 30, 3 );
							}
					...
					case 'unsubscribe_student_link': 
						if(!empty($this->settings['unsubscribe_student_link'])){
							add_action('wplms_user_course_stats',array($this,'add_unsubscribe_link'),10,2);
							add_action('wp_footer',array($this,'add_unsubscribe_link_script'));
							add_action('wp_ajax_wplms_unsubscribe_me_course',array($this,'wplms_unsubscribe_me_course'));
						}
					case 'instructor_login_redirect': 
						if(!empty($this->settings['instructor_login_redirect'])){
							...
							//add_filter('vibebp_login_redirect_component',array($this,'login_redirect_v4'));
							add_filter('vibebp_login_redirect_component',array($this,'login_redirect_v4_2'),11,2);
						}
					break;
					...
					case 'course_curriculum_below_description':
						if(function_exists('is_wplms_4_0') && !is_wplms_4_0('course')){
					...
					case 'unit_quiz_start_datetime':
						...
						add_filter('wplms_course_creation_tabs',array($this,'show_unit_date_time_4'));
					...
					case 'disable_ajax':
						...
						add_filter('wplms_plugin_get_course_unfinished_unit',array($this,'load_unit'));
						add_filter('wplms_plugin_get_course_unfinished_unit_key',array($this,'unit_key'),10,3);
					break;
					case 'course_codes':
						...
						add_filter('wplms_course_creation_tabs',array($this,'course_codes_setting_front'),11);
					...
					case 'enable_forum_privacy':
						...
						add_filter('vibe_helpdesk_forums_args',array($this,'check_forum_privacy_helpdesk'),10,2);

						add_filter('vibe_helpdesk_topic_args',array($this,'check_topic_privacy_helpdesk'),10,2);
						add_filter('vibe_helpdesk_replies_args',array($this,'check_reply_privacy_helpdesk'),10,2);
					...
					break;
					case 'remove_expired_course':
							add_filter('wplms_carousel_course_filters',array($this,'remove_expired_course'));
							add_filter('wplms_grid_course_filters',array($this,'remove_expired_course'));
							add_filter('vibe_editor_filterable_type',array($this,'remove_expired_course'));
							add_filter('bp_course_wplms_filters',array($this,'remove_expired_course'));
					break;
					case 'course_coming_soon':
						...
						add_filter('wplms_course_creation_tabs',array($this,'wplms_coming_soon_front'),11);
					...
					case 'quiz_passing_score':
						add_filter('wplms_next_unit_access',array($this,'wplms_next_access'),10,3);
						...
						add_filter('wplms_finish_course_check',array($this,'check_last_quiz_pass_fail'),10,3);
						add_filter('wplms_course_creation_tabs',array($this,'add_quiz_passing_score'));
					break;
					case 'disable_react_quizzes':
						if(!empty($this->settings['disable_react_quizzes'])){
							add_filter('wplms_use_react_quizzes',function(){
								return 0;
							});
							//add_filter('wplms_front_end_quiz_settings',array($this,'question_number_react_control'));
							//add_action('wplms_front_end_quiz_settings_action',array($this,'question_number_react_setting'),10,1);
						}
					break;
					case 'question_advanced_stats':
						if(!empty($this->settings['question_advanced_stats'])){
							add_filter('show_question_advanced_stats_in_quiz',function($x){
								return 1;
							},10);
						}
					break;
					...
					case 'quiz_negative_marking':
						...
						add_filter('wplms_course_creation_tabs',array($this,'wplms_quiz_negative_marking_4_0'));
					...
					case 'course_external_link': 
						...
						add_filter('wplms_course_creation_tabs',array($this,'wplms_external_link_front'),11);
					...
					break;
					case 'completed_unit_link_course_status_for_all_units':
						add_filter('wplms_curriculum_course_link',array($this,'wplms_curriculum_course_link_for_all_units'),10,3);
						add_action('template_redirect',array($this,'remove_unit_restriction_hook_for_all_units'),1);
					break;
					case 'randomize_question_options':
						if(is_wplms_4_0()){
							add_filter('wplms_question_option_rearrange',function($x){return 1;});
						}else{
	...
	function wplms_commissions_migrate_to_activity($x){
		return 0;
	}
	...
	function wplms_guest_popup_filter($html){
		...
		$partial = get_post_meta($post->ID,'vibe_partial_free_course',true);
		if(!(function_exists('vibe_validate') && (vibe_validate($partial) || vibe_validate($free))))
			return $html;
		ob_start();
		wp_nonce_field('login_as_guest','login_as_guest');

		?>
		<a href="javascript:void();" class="link text-center login_as_guest"><?php echo _x('Or Pursue As Guest','','wplms')?></a>
		<script>
		        ...
		        $('body').delegate('.login_as_guest','click',function(){
		        	...
		        	jQuery.ajax({
				        ...
				        success: function (html) {
				          if(html == 'success'){
				          	window.location.href = '<?php echo ((function_exists('vibe_validate') && vibe_validate($partial))?get_permalink($post->ID).'?subscribe':get_permalink($post->ID));?>';
				          }
				        }
				    });
		        });
	...
	function trigger_notice(){
		if (function_exists('wp_get_theme')){
			...
			if($theme_data->template != 'wplms' && $theme_data->template != 'wplms/wplms' && $theme_data->stylesheet != 'wplms' && $init->message()){
				echo '<div class="message error">'.$init->message().'</div>';
			}
	...
	function do_not_allow_profile_but_results(){
		...
		if(function_exists('bp_is_my_profile') &&  bp_is_my_profile() && $this->settings['guest_user_id'] == get_current_user_id()){
			if(bp_current_action() != 'course-results'){
				wp_die(_x('Your are a Guest. Guests do not have any profile','','wplms'));
			}
			
		}
	}

	function add_unsubscribe_link($user_id,$course_id){
		
		?>
		<a href="javascript:void(0);" class="link unsubscribe_me_course" data-course="<?php echo $course_id;?>"><?php echo _x('Unsubscribe from course','','wplms')?></a>
		<?php
	}

	function add_unsubscribe_link_script(){
		if(!is_user_logged_in())
			return;
		$user_id = get_current_user_id();
		wp_nonce_field('wplms_unsubscribe_me_course'.$user_id,'wplms_unsubscribe_me_course');
		?>
		<script>
			jQuery(document).ready(function($){
				$('.unsubscribe_me_course').on('click',function (){
					var course = $(this).data('course');
					if(!course){
						alert('<?php echo _x('Course missing.Could not unsubscribe!','','wplms');?>');
					}else{
						var un_cn_text = '<?php echo _x('Are you sure you want to unsubscribe from course','','wplms');?>';
				        $.confirm({
				          text: un_cn_text,
				          confirm: function() {
				            jQuery.ajax({
						        ...
						        url: ajaxurl,
						        dataType: 'json',
						        data: { action: 'wplms_unsubscribe_me_course', 
						        		course_id:course,
						                security: $('#wplms_unsubscribe_me_course').val(),
						              },
						        cache: false,
				          ...
				          confirmButton: '<?php echo _x('Yes','','wplms');?>',
				          cancelButton: '<?php echo _x('Cancel','','wplms');?>'
				        });
	...
	function wplms_unsubscribe_me_course(){
		if(!is_user_logged_in()){
			$data = array('status'=>false,'message'=>_x('User not logged in','','wplms'));
			echo json_encode($data);
			die();
		}
		$user_id = get_current_user_id();
		$course_id = $_POST['course_id'];
		if ( !isset($_POST['security']) || !wp_verify_nonce($_POST['security'],'wplms_unsubscribe_me_course'.$user_id) ){
			$data = array('status'=>false,'message'=>_x('Security check failed','','wplms'));
			echo json_encode($data);
			die();
		}
		if(!empty($course_id ) && is_numeric($course_id )){
			bp_course_remove_user_from_course($user_id,$course_id);
			$data = array('status'=>true,'message'=>_x('Successfully unsubscribed from course!','','wplms'));
		}else{
			$data = array('status'=>false,'message'=>_x('Something went wrong!','','wplms'));
		}
		echo json_encode($data);
		die();
	...
	function wplms_curriculum_course_link_for_all_units($show_link,$unit_id,$course_id){
		if(!is_user_logged_in()){
			return $show_link;
		}
		...
		if(bp_course_is_member($course_id, $user_id) && !empty($status) && $status < 3){
			if(!empty($this->settings['show_unit_title_course_status'])){
				$link = get_permalink($course_id);
				$link .= get_post_field('post_name',$unit_id);
			}else{
	...
	function remove_unit_restriction_hook_for_all_units(){
	    if(!is_singular(array('unit')))
	      return;
  		$actions = WPLMS_Actions::init();
  		//check if show_unit_title_course_status is not enabled only then remove
	...
	function filter_course_unit($vars){
		...
		$unit_id = $wpdb->get_var($wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE post_name = %s AND (post_type = 'unit' OR post_type='quiz' OR post_type='wplms-assignment') AND post_status='publish'",$name[1]));
	...
	*/
	...
	function pdf_certificates($url,$course_id,$user_id){
		...
		$certificate_pdf = $wpdb->get_var("SELECT ID FROM {$wpdb->posts} WHERE post_type ='attachment' AND post_author = $user_id AND post_name = '".$name."'");

		if(!empty($certificate_pdf)){
			return wp_get_attachment_url($certificate_pdf);
		}
	...
	function check_course_student($link,$quiz_id){
		...
		if(!empty($course_id) && is_user_logged_in() && !current_user_can('edit_post',$quiz_id)){
			...
			if(!bp_course_is_member($course_id)){
				return '<a href="'.get_permalink($course_id).'" class="button create-group-button full"> '.__('Take the Course to Start the Quiz','wplms').'</a>';
			}
		}
		return $link;
	}
	...
	*/
	function remove_expired_course($args){
  		...
  		foreach($courses as $course_id){
  			if(function_exists('bp_course_get_user_expiry_time')){
      	...
      	if(!empty($expired_courses) && is_array($expired_courses) && !isset($args['author'])){
      		if(!empty($args['post__not_in'])){
      			$args['post__not_in'] = array_merge($args['post__not_in'], $expired_courses);
      		}else{
      			$args['post__not_in'] = $expired_courses;	
      		}
      	} 
    ...
    */
	function terms_conditions_validation(){
        ...
        if (empty($custom_field) || $custom_field == '') {
            $bp->signup->errors['terms_conditions'] = __('Please Check Terms & Conditions','wplms');
        }
        return;
	}
	/*
    ...
    */
	function show_terms_conditions(){
        ...
        echo '<input type="checkbox" name="terms_conditions" id="bph_field" value="1" /> <strong>'.__(' I agree to these Terms and Conditions','wplms').'</strong>
    ...
    */
	function disable_ajax($course_id){
		...
		$status = 0;
		$user_id = 0;
		if(function_exists('bp_course_get_user_course_status') && is_user_logged_in()){
			$user_id = get_current_user_id();
			$status = bp_course_get_user_course_status($user_id,$course_id);
		}
		if(!empty($status) && $status==1){
		...
		if(isset($assignments) && is_array($assignments) && count($assignments)){
			foreach($assignments as $assignment){
				...
				<tr class="course_lesson">
					...
					<td><span class="time"><i class="fa fa-clock-o"></i><?php echo ($duration > 9998 ? _x('UNLIMITED','wplms','assignment duration in curriculum') : (empty($days)?'':$days.', ').(empty($hours)?'00':sprintf('%1$02d',$hours)).':'.(empty($minutes)?'00':sprintf('%1$02d',$minutes))); ?></span></td>
				</tr>
				<?php
			}
		}
	...
	function wplms_next_access($access,$quiz_id,$user_id=null){
		if(empty($user_id))
			$user_id = get_current_user_id();
		...
		foreach($metabox as $key=>$value){
			if($key == 'vibe_quiz_questions'){
				$newmetabox['vibe_quiz_passing_score'] = array( // Text Input
					'label'	=> __('Quiz passing marks/score','wplms'), // <label>
					'desc'	=> __('Passing marks/score for quiz. Combined with Prev.Unit/Quiz lock the user progress can be restricted.','wplms'), // description
	...
	function add_quiz_passing_score($tabs){
	...
	function quiz_passing_score_setting($setting){
		?>
		<li><label><?php _e('SET QUIZ PASSING SCORE','wplms'); ?></label>
	...
	function check_last_quiz_pass_fail($flag,$course_curriculum,$user_id=null){
		if(empty($user_id)){
			$user_id=get_current_user_id();
		}
	...
	function negative_marking_setting($setting){
		?>
		<li><label><?php _e('NEGATIVE MARKS PER QUESTION','wplms'); ?></label>
	...
	function skip_question_switch($type,$question_id){
		echo '<a class="clear_question_marked_answer" data-id="'.$question_id.'" title="'.__('Clear marked answer','wplms').'"><i class="fa fa-trash"></i></a>';
			?><script>
				jQuery('.clear_question_marked_answer').on('click',function(){
				var id = jQuery(this).attr("data-id");
		      	localStorage.removeItem(id); 
		      	jQuery('.in_quiz').trigger('question_loaded');
		...
		foreach($metabox as $key=>$value){
			if($key == 'vibe_quiz_questions'){
				$newmetabox['vibe_quiz_negative_marks_per_question'] = array( // Text Input
					'label'	=> __('Negative Marks per Question','wplms'), // <label>
					'desc'	=> __('Deduct marks for a Wrong answer.','wplms'), // description
	...
	function wplms_quiz_negative_marking_4_0($tabs){
		$setting = array( // Text Input
					'label'	=> __('Negative Marks per Question','wplms'), // <label>
					'desc'	=> __('Deduct marks for a Wrong answer.','wplms'), // description
					'id'	=> 'vibe_quiz_negative_marks_per_question', // field id and name
	...
	function coming_soon_display($credits,$course_id){
		$coming_soon = get_post_meta($course_id,'vibe_coming_soon',true);
		if(vibe_validate($coming_soon)){
			return '<strong><span class="coming_soon">'.__('COMING SOON','wplms').'</span></strong>';
		}
		return $credits;
	}
	...
	function wplms_coming_soon_backend($metabox){
		$metabox[] = array( // Text Input
					'label'	=> __('Coming soon Mode','wplms'), // <label>
					'desc'	=> __('Enable Coming soon mode','wplms'), // description
	...
	function wplms_coming_soon_front($setting){
		$set =  array(array( // Text Input
					'label'	=> __('Coming soon Mode','wplms'), // <label>
					'desc'	=> __('Enable Coming soon mode','wplms'), // description
					'id'	=> 'vibe_coming_soon', // field id and name
		...
		array_splice($setting['course_pricing']['fields'],(count($setting)-2),0,$set);
	...
	function wplms_coming_soon_front_end_pricing($course_id){
		...
		echo '<li>
                <h3>'.__('Coming Soon mode','wplms').'<span>
                    <div class="switch coming_soon">
                            ...
                            <label for="disable_coming_soon" class="switch-label switch-label-off">'.__('Disable','wplms').'</label>
                            ...
                            <label for="enable_coming_soon" class="switch-label switch-label-on">'.__('Enable','wplms').'</label>
	...
	function bpp_get_restricted_forum_ids($user_id=null){
	...
	function check_forum_privacy_helpdesk($args,$user){
		$forums_ids =  $this->bpp_get_restricted_forum_ids($user->id);
		if(!empty($forums_ids)){
			if(!empty($args['post__not_in'])){
				$args['post__not_in'] = array_merge($args['post__not_in'],$forums_ids);
				$args['post__not_in'] = array_unique($args['post__not_in']);
			}else{
				$args['post__not_in'] = $forums_ids;
			}
		}
		return $args;
	}

	function check_topic_privacy_helpdesk($args,$user){
		$forums_ids =  $this->bpp_get_restricted_forum_ids($user->id);
		if(!empty($forums_ids)){
			if(!empty($args['post_parent__not_in'])){
				$args['post_parent__not_in'] = array_merge($args['post_parent__not_in'],$forums_ids);
				$args['post_parent__not_in'] = array_unique($args['post_parent__not_in']);
			}else{
				$args['post_parent__not_in'] = $forums_ids;
			}
		}
		return $args;
	}

	function check_reply_privacy_helpdesk($args,$user){
		$forums_ids =  $this->bpp_get_restricted_forum_ids($user->id);
		if(!empty($forums_ids)){
			$topic_ids = [];
			$rp_args = array(
				'post_type'=>'topic',
				'posts_per_page'=>-1,
				'post_parent__in'=>$forums_ids,
				'post_status' => 'publish'
			);
			$results = new WP_Query($rp_args);

		   	if($results->have_posts()){
			    while($results->have_posts()){
			        $results->the_post();
		        	$topic_ids[] = get_the_ID();
			...
			if(!empty($topic_ids)){
				if(!empty($args['post_parent__not_in'])){
					$topic_ids = array_merge($forums_ids,$topic_ids);

					$args['post_parent__not_in'] = array_merge($args['post_parent__not_in'],$topic_ids);
					...
					$topic_ids = array_merge($forums_ids,$topic_ids);
					$args['post_parent__not_in'] = $topic_ids;
				}
			}
	...
	function bpp_can_user_view_post($post_id){
		...
		if(!is_array($instructors)){$instructors=array($instructors);}

		if(empty($course_id) ||  in_array($user_id, $instructors))
			return true;
    ...
    function register_student_menus() {
	    register_nav_menus(
    	    array(
	            'student-top-menu' => __( 'Top Menu for Students','wplms' ),
	            'student-main-menu' => __( 'Main Menu for Students','wplms' ),
	            'student-mobile-menu' => __( 'Mobile Menu for Students','wplms' ),
	            )
        );
    }
    function register_instructor_menus() {
	    register_nav_menus(
    	    array(
	            'instructor-top-menu' => __( 'Top Menu for Instructor','wplms' ),
	            'instructor-main-menu' => __( 'Main Menu for Instructor','wplms' ),
	            'instructor-mobile-menu' => __( 'Mobile Menu for Instructor','wplms' ),
	            )
        );
    }
    ...
    function create_course_setup_nav_instructor(){
	    ...
	    if(bp_is_my_profile() && current_user_can('edit_posts')){
	       $check =1;
	       bp_core_new_nav_item( apply_filters('vibe_create_course_setup_nav_instructor',array(
	           'name' => __('Create a Course', 'wplms' ),
	           'slug' => 'create-course',
	           'position' => 100,
	...
	function wplms_ninja_forms_sub_table_row_actions_convert_to_instructor( $row_actions, $data, $sub_id, $form_id ) {
	    ...
	    if(isset($ninja_instructor_form_id) && $ninja_instructor_form_id == $form_id){
	    	$row_actions['instructor'] = apply_filters('wplms_ninja_forms_change_to_instructor_sub','<span><a href="?post_status=all&post_type=nf_sub&action=-1&form_id='.$form_id.'&make_instructor" id="'.$sub_id.'" class="wplms_ninja_forms_convert_to_instructor_sub">'. __( 'Make Instructor', 'wplms' ).'</a></span>',$sub_id,$form_id);
	    }
	  return $row_actions;
		              ...
		              if(!user_can($user->ID,'edit_posts')){
		                $user_id = wp_update_user( array( 'ID' => $user_id, 'role' => 'instructor' ) );
		                  if ( is_wp_error( $user_id ) ) {
		                    return '<span>'.__('There was some error','wplms').'</span>';
		                  } else {
		                    return '<span><a href="?post_status=all&post_type=nf_sub&action=-1&form_id='.$form_id.'&make_student" id="'.$sub_id.'" class="wplms_ninja_forms_convert_to_instructor_sub">'. __( 'Make Student', 'wplms' ).'</a></span>';
		                  }
		              }else{
		              	return '<span>'.__('Instructor','wplms').'</span>';
		              }
		            }
		       }
		    }
		...
		if(is_numeric($instructor_id) && is_user_logged_in()){ //removed  && is_singular('course')
			...
			if(is_user_logged_in()){
				...
				if($user_id != $instructor_id && function_exists('bp_get_messages_slug')){
					...
					$meta .= '<li><a href="'.$link.'" class="button tip" title="'.__('Send Message','wplms').'"><i class="icon-email"></i></a></li>';
				}
			}
			$user_info = get_userdata($instructor_id);
			$meta .= '<li><a href="mailto:'.$user_info->user_email.'" class="button tip" title="'.__('Send Email','wplms').'"><i class="icon-at-email"></i></a></li>';
			$meta .= '</ul>';
		}
		return $meta;
	}
	...
	function free_label($label,$course_id){
		$free = get_post_meta($course_id,'vibe_course_free',true);
		if(vibe_validate($free)){
		 $label =apply_filters('wplms_take_this_course_button_label',__('TAKE THIS COURSE','wplms'),$course_id).apply_filters('wplms_course_button_extra','',$course_id);
		}
		return $label;
	}
       ...
       $course_id =get_the_ID();
        ...
        if(isset($_POST['submit_course_codes'])){
             if ( !isset($_POST['security_code']) || !wp_verify_nonce($_POST['security_code'],'security'.$user_id) ){
             	$this->course_code_message = '<p class="message">'.__('Security check Failed. Contact Administrator.','wplms').'</p>';
             	return;
            }else{
                $code = $_POST['course_code'];
                $course_codes = get_post_meta($course_id,'vibe_course_codes',true);
                if(isset($code) && strlen($code)<2 || (strpos($code,'|') !== false)){
                    $this->course_code_message = '<p class="message">'.__('Code does not exist. Please check the code.','wplms').'</p>';
                    return;
                }
                    ...
                    if(isset($matches[3]) && is_numeric($matches[3])){
                        ...
                        if($count < $total_count){
                            if(!wplms_user_course_check($user_id,$course_id)){
                                ...
                                $this->course_code_message = '<p class="message success">'.__('Congratulations! You are now added to the course.','wplms').'</p>';
                            }else{
                                $this->course_code_message = '<p class="message">'.__('User already in course.','wplms').'</p>';
                            }
                        }else{
                            $this->course_code_message = '<p class="message">'.__('Maximum number of usage for course code exhausted','wplms').'</p>';
                        }
                    }else{
                        if(!wplms_user_course_check($user_id,$course_id)){
                            do_action('wplms_course_code',$code,$course_id,$user_id);
	...
	function wplms_front_end_show_course_codes($course_id){
		...
		echo '<li class="course_membership"><h3>'.__('Course Codes','wplms').'<span>
                  <textarea id="vibe_course_codes" class="vibe_extras" placeholder="'.__('Enter Course codes (XXX|2,YYY|4)','wplms').'" >'.$course_codes.'</textarea>
              </span>
              </h3>
          </li>';
	}
	...
	function course_codes_setting_front($setting){
		$arr=array(array( // Text Input
					'label'	=> __('Set Course purchase codes','wplms'), // <label>
					'desc'	=> __('Student can gain access to Course using course codes (multiple codes comma separated, usage count pipe saperate eg : xxx|2,yyy|4)','wplms'), // description
					'id'	=> 'vibe_course_codes', // field id and name
					'type'	=> 'textarea', // type of field
					'from' => 'meta',
				));
		array_splice($setting['course_pricing']['fields'],(count($setting)-2),0,$arr);
	...
	function show_unit_date_time_backend($settings){
		...
		$settings[]= array( // Text Input
					'label'	=> __('Access Date','wplms'), // <label>
					'desc'	=> __('Date on which unit is accessible','wplms'), // description
					'id'	=> $prefix.'access_date', // field id and name
					'type'	=> 'date', // type of field
				);
		$settings[]=array( // Text Input
					'label'	=> __('Access Time','wplms'), // <label>
					'desc'	=> __('Time after which unit is accessible','wplms'), // description
	...
	function show_unit_date_time_4($tabs){
		...
		$setting[] = array( // Text Input
					'label'	=> __('Access Date','wplms'), // <label>
					'desc'	=> __('Date on which unit is accessible','wplms'), // description
					'id'	=> 'vibe_access_date', // field id and name
					'type'	=> 'date', // type of field
					'from'  => 'meta',
				);
		$setting[] = array( // Text Input
					'label'	=> __('Access Time','wplms'), // <label>
					'desc'	=> __('Time after which unit is accessible','wplms'), // description
					'id'	=> 'vibe_access_time', // field id and name
					'type'	=> 'time', // type of field
					'from'  => 'meta',
				);
		foreach ($tabs['course_curriculum']['fields'] as $key => $field) {
	...
	function show_date_time_field($unit_settings){
		...
		jQuery(document).ready(function(){
		     ...
		     <li><label>'.__('Unit access date','wplms').'</label>
                <h3>'.__('Access date','wplms').'<span>
                <input type="text" class="datepicker vibe_extras" id="vibe_access_date" value="'.$unit_settings['vibe_access_date'].'" /> 
            </li><li><label>'.__('Unit access time','wplms').'</label>
                <h3>'.__('Access time','wplms').'<span>
	...
	function login_redirect_v4_2($r,$expanded_token=null){
		$site_lock_url = '';
		...
		if(function_exists('vibebp_get_setting') && vibebp_get_setting('bp_single_page')){
			$purl = get_permalink(vibebp_get_setting('bp_single_page'));
		}
		$redirect_array = apply_filters('wplms_redirect_location',array(
			'home' => $url,
			'profile' => '#',
			'mycourses' => $purl.'#component=course',
			'instructing_courses' => $purl.'#component=course&action=manage_courses',
			'dashboard' => $purl.'#component=dashboard',
			'same' => false,
		));
		if(!empty($expanded_token) && !empty($expanded_token->data->user)){
			$role = $this->check_user_role($expanded_token->data->user->caps);

			if(!empty($redirect_array[$this->settings[$role.'_login_redirect']])){
				return $redirect_array[$this->settings[$role.'_login_redirect']];
			}
		}
		

		if(!empty($redirect_array[$this->settings['instructor_login_redirect']])){
			return $redirect_array[$this->settings['instructor_login_redirect']];
		}
		return $r;
	}

	function login_redirect_v4($r){
		...
		$redirect_array = apply_filters('wplms_redirect_location',array(
			'home' => $url,
			'profile' => '#',
			'mycourses' => '#component=course',
			//'instructing_courses' => '#component=course&action=manage_courses',
	...
	*/
	function course_creator_curriculum_accordion(){
		...
		if(!is_page($cc_id) || !current_user_can('edit_posts') || is_wplms_4_0())
			return;
				...
				<td colspan="4">
					<?php 
						if(!empty($description)){
							...
							echo __('No Description Found','wplms');
						}
					?>
				</td>
			</tr>
	...
	function one_session_per_user( $user, $username, $password ) { 
		...
		if ( count($all_sessions) ) {
			...
			if($flag)
				$user = new WP_Error('already_signed_in', __('<strong>ERROR</strong>: User already logged in.','wplms'));
		}
	    return $user;
	}

	function one_session_per_user_check_guest( $user, $username, $password){
		if($this->settings['guest_user_id'] == $user->ID){
			return $user;
		}
	...
	* 
	...
	function course_external_link_setting($settings){
		$settings['vibe_course_external_link'] =array( // Text Input
			'label'	=> __('External Course button link','wplms'), // <label>
			'text'   => __('Connect Course button to an external page (Leave blank to skip)','wplms'),
			'desc'	=> __('Enter the external page url. Leave blank or 0 to skip','wplms'), // description
	...
	function wplms_external_link_front($setting){
	...
	*/
	function show_course_badge_popup_in_course_details($html,$course_id){
		...
		$batdge_title = get_post_meta($course_id,'vibe_course_badge_title',true);
  		...
  		$html = '<li class="course_badge"><a class="tip ajax-badge" data-course="'.get_the_title($course_id).'" title="'.$batdge_title.'"><img style="display:none;" src="'.$badge_url[0].'" title="'.$badge['title'].'"/><i class="icon-award-stroke"></i> '.__('Course Badge','wplms').'</a></li>';
	...
	*/
	function show_course_certificate_popup_in_course_details($html,$course_id){
		...
		$html = '<li class="course_certificate"><a href="'.$certificate_url.'" class="ajax-certificate  regenerate_certificate" data-user="'.$author_id.'" data-course="'.$course_id.'"><i class="icon-certificate-file"></i>  '.__('Course Certificate','wplms').'</a></li>';
	...
	*/
		...
		<script>
			jQuery(document).ready(function($){
				$('.unit_content').on('unit_traverse',function(){
				  ...
				  let time = parseInt($('span.unit_duration').attr('data-duration'));
	...
	function calculate_course_duration_from_start_course($course_id,$user_id){
		...
		if($course_status != 1)
			return;
		$time = $expiry = $seconds = $bulk_action_time  = 0;
		...
		$bulk_action_time = $wpdb->get_var($wpdb->prepare( "
        ...
        if(!empty($bulk_action_time)){
            $bulk_action_time = strtotime($bulk_action_time);
        }else{
        	$bulk_action_time = 0;
        }

        if(!empty($bulk_action_time) && $bulk_action_time > $time){
        	
        	$time = $bulk_action_time;
        }
		...
		if($time && time() > $time){
			...
			if(function_exists('bp_course_update_user_expiry_time')){
...
class wplms_course_codes extends WP_Widget {
	...
	function __construct() {
	    $widget_ops = array( 'classname' => 'wplms_course_codes', 'description' => __('WPLMS Course codes widget', 'wplms') );
	    $control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'wplms_course_codes' );
	    parent::__construct( 'wplms_course_codes', __('WPLMS Course Codes', 'wplms'), $widget_ops, $control_ops );
  	}
        
    function widget( $args, $instance ) {
    	global $post;
    	if($post->post_type != BP_COURSE_CPT || !defined('BP_COURSE_CPT') || !is_user_logged_in())
    		return;
    	...
    	if ( $title )
      	...
      	echo '<input type="submit" name="submit_course_codes" value="'.__('Submit','wplms').'"/></form>';
    	echo $after_widget;
    }
    ...
    $defaults = array( 
        'title'  => __('Enter Course code','wplms'),
        'placeholder'  => __('Place holder text','wplms'),
        'hide_for_members'=>0,
    );
    $instance = wp_parse_args( (array) $instance, $defaults );                 
    ?>
    <p> <?php _e('Title','wplms'); ?> <input type="text" class="text" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo $instance['title']; ?>" /></p>
    <p> <?php _e('Course Codes input box text','wplms'); ?> <input type="text" class="text" name="<?php echo $this->get_field_name('placeholder'); ?>" value="<?php echo $instance['placeholder']; ?>" /></p>
    <p> <?php _e('Hide Course codes for Course members','wplms'); ?> <input type="checkbox" name="<?php echo $this->get_field_name('hide_for_members'); ?>" value="1" /></p>
	<?php
    }
