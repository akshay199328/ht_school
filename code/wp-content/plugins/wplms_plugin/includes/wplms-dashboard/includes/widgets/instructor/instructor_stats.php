...
class wplms_instructor_stats extends WP_Widget {
  ...
  function __construct() {
    $widget_ops = array( 'classname' => 'wplms_instructor_stats', 'description' => __('Instructor Statistics widget', 'wplms') );
    $control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'wplms_instructor_stats' );
    parent::__construct( 'wplms_instructor_stats', __(' DASHBOARD : Instructor Stats', 'wplms'), $widget_ops, $control_ops );
    ...
    function enqueue_script(){
        ...
        if(is_user_logged_in()){
            ...
            wp_enqueue_script('wplms_dashboard_instructor_stats',WPLMS_PLUGIN_URL.'/assets/js/instructorstats.js',array('wp-element','wp-data'),WPLMS_PLUGIN_VERSION,true);
            wp_enqueue_style('wplms_dashboard_css',WPLMS_PLUGIN_URL.'/assets/css/dashboard.css',array(),WPLMS_PLUGIN_VERSION);
          wp_localize_script('wplms_dashboard_instructor_stats','instructor_stats',array(
            'settings'=>array(),
            ...
            'translations'=>array(
              ...
              'average' => __('Average','wplms'),
              'show_stats_graph'=> __('Show graph','wplms'), 
              'reload_stats'=> __('Reload stats','wplms'), 
              'load_curriculum'=> __('Load curriculum','wplms'),
            )
          ));
        }
        }
    ...
    function widget( $args, $instance ) {
      ...
      if(is_wplms_4_0()){
            echo '<div class="instructor_stats"></div>';
    ...
    function form($instance) {  
        $defaults = array( 
                        'title'  => __('Instructor Statistics','wplms'),
        ...
        <p>
          <label for="<?php echo $this->get_field_id('range'); ?>"><?php _e('Set Range value (10 for 1/10)','wplms'); ?></label> 
        ...
        <p>
          <label for="<?php echo $this->get_field_id('max'); ?>"><?php _e('Set Max course limit','wplms'); ?></label> 
    ...
    function generate_ranges(){
      ...
      $query = $wpdb->get_results($wpdb->prepare("
           ...
           echo json_encode('<p class="message">'.__('No data available','wplms').'</p>');
          }
          die();  
    }
    function load_quiz_assignment_list(){
      ...
      if(is_array($quiz_list) || is_array($assignment_list)){
        ...
        foreach($quiz_list as $quiz){
         echo '<li><strong class="quiz_label">'.__('Quiz','wplms').'</strong> : '.$quiz['title'].'<span data-id="'.$quiz['id'].'" class="list-stats"><i class="icon-bars "></i></span></li>';
        }
        if(is_array($assignment_list))
        foreach($assignment_list as $assignment){
         echo '<li><strong class="assignment_label">'.__('Assignment','wplms').'</strong> : '.$assignment['title'].'<span data-id="'.$assignment['id'].'" class="list-stats"><i class="icon-bars "></i></span></li>';
        }
        echo '</ul>';
      }
      die();
