...
add_action( 'widgets_init', 'wp_bootstrap_starter_widgets_init' );
  ...
  if(!is_admin()){
    ...
    if (is_category() || is_page_template('blog.php') || is_single() || is_page_template('testimonial-listing.php')){
...
function get_the_term_list_search( $post_id, $taxonomy, $before = '', $sep = '', $after = '' ) {
  ...
  foreach ( $terms as $term ) {
    ...
    $links[] = '<li><a href="' . $homeurl . '" rel="tag">' . $term->name . '</a></li>';
  }
...
/* AJAX Function to load more Sessions - Added by Komal */  
 ...
 $course_curriculum = ht_course_get_full_course_curriculum($course_id); 
  ...
  foreach($course_curriculum as $lesson)
  {
   ...
   /*if($countunit>$session_limit)
    ...
    <a class='view-all load-more'>".$btnName."</a>
    <input type='hidden' id='sessionCount' value='".$countunit."'>
