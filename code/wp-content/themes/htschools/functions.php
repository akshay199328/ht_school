...
$check_email_id = $_REQUEST['check_student_email'];

  global $wpdb;
  $user_id = get_current_user_id();

  $result_id = $wpdb->get_results("SELECT DISTINCT student_email FROM ht_school_response_data WHERE student_email = '" . esc_attr($check_email_id) . "'");
...
$check_parent_email = $_REQUEST['check_parent_email'];

  global $wpdb;
  $user_id = get_current_user_id();

  $result_id = $wpdb->get_results("SELECT DISTINCT parent_email FROM ht_school_response_data WHERE parent_email = '" . esc_attr($check_parent_email) . "'");
...
add_action("wp_ajax_check_check_student_contact_number", "check_student_contact_number");
add_action( 'wp_ajax_nopriv_check_student_contact_number', 'check_student_contact_number' );

function check_student_contact_number(){
$check_stud_contact_number = $_REQUEST['check_mobile_number'];

  global $wpdb;
  $user_id = get_current_user_id();

  $result_id = $wpdb->get_results("SELECT DISTINCT student_contact_no FROM ht_school_response_data WHERE student_contact_no = " . esc_attr($check_stud_contact_number) . "");
  $get_stud_contact_no = $result_id[0]->student_contact_no;
  
  if($get_stud_contact_no != ''){
    $response = array(
      'status' => 1,
      'response' => $get_stud_contact_no
    );
...
$check_parent_contact_number = $_REQUEST['check_parent_contact'];

  global $wpdb;
  $user_id = get_current_user_id();

  $result_id = $wpdb->get_results("SELECT DISTINCT parent_contact_no FROM ht_school_response_data WHERE parent_contact_no = " . esc_attr($check_parent_contact_number) . "");
