...
class WPLMS_Actions{
    ...
    private function __construct(){
        ...
        add_action('wplms_single_course_content_end',array($this,'show_related_courses'));
    ...
    function add_loading_css(){
    	...
    	$page_loader = vibe_get_option('page_loader');
   		if(!empty($page_loader) && !is_customize_preview()){
	    	...
	    	?>
	    	<style>
	    	body.loading .pusher:before, body.bodyloader .pusher:before{content:'';position:fixed;left:0;top:0;width:100%;height:100%;background:rgba(255,255,255,.95);z-index:999}body.loading.pageloader1 .global:after,body.loading.pageloader1 .global:before, .bodyloader.pageloader1 .global:after,body.bodyloader.pageloader1 .global:before{content:'';position:fixed;left:50%;top:50%;margin:-20px 0 0 -20px;width:40px;height:40px;border-radius:50%;z-index:9999;border:4px solid transparent;border-top-color:#fade3b;z-index:9999;animation:rotate linear 1.5s infinite}body.loading.pageloader1 .global:after, body.bodyloader.pageloader1 .global:after{margin:-27px 0 0 -27px;width:54px;height:54px;border-top-color:transparent;border-left-color:#fade3b;animation:rotate linear 1s infinite}@keyframes rotate{0%{transform:rotate(0)}50%{transform:rotate(180deg)}100%{transform:rotate(360deg)}}
			</style>
	    	<?php
	    	}else{
	    	?>
	    	<style>
	    	body.loading .pusher:before, body.bodyloader .pusher:before{content:'';position:fixed;left:0;top:0;width:100%;height:100%;background:rgba(255,255,255,.95);z-index:999}body.loading.pageloader2 .global:after,body.loading.pageloader2 .global:before, body.bodyloader.pageloader2 .global:after,body.bodyloader.pageloader2 .global:before{content:'';position:fixed;left:50%;top:50%;margin:-8px 0 0 -8px;width:15px;height:15px;border-radius:50%;z-index:9999;background:#fade3b;z-index:9999;animation:flipzminus linear 1s infinite}body.loading.pageloader2 .global:after, body.bodyloader.pageloader2 .global:after{animation:flipzplus linear 1s infinite}@keyframes flipzminus{0%{transform:translateX(-30px);opacity:1}50%{transform:translateX(0);opacity:.5}100%{transform:translate(30px);opacity:1}}@keyframes flipzplus{0%{transform:translate(30px);opacity:1}50%{transform:translateX(0);opacity:.5}100%{transform:translateX(-30px);opacity:1}}
	    	...
	    	<script> 
    			...
    			if(suser){
    				...
    				if(Object.keys(user).length){
	    				document.querySelector('.vibebp-login').innerHTML='<div class="loggedin_user_div"><a class="loggedin_user" data-id="1"><img src="'+user.avatar+'"><span class="vibebp_name">loading</span></a><div class="user_menu active"><div class="usermenu_content"><a href="/members-directory/'+user.user_nicename+'">Profile </a></div><a class="vibebp-logout" href="<?php echo wp_logout_url( get_permalink() ); ?>"><span class="vicon"></span>Logout </a></div></div>';
  ...
  function show_related_courses(){
    ...
    <?php
     ...
     <div class="list mobile-slider">
      ...
      echo '<div class="col-sm-12 col-lg-4 mrg card item">';

      if(empty($style))
        $style = 'course_related';

      echo thumbnail_generator($post,'course_related','medium');
      echo '</div>';
      endwhile;
      ?>
      </div>
