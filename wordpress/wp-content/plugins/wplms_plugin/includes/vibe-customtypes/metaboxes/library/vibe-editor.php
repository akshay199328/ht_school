...
class WPLMS_Page_Builder{
    ...
    private function __construct(){
        ...
        add_shortcode('wplms_carousel', array($this,'wplms_custom_post_carousel'));
        ...
        $columns=['1_2','1_3','1_4','1_4_2','2_3','3_4','resizable','stripe_container','stripe'];
        foreach($columns as $column_key){
          add_shortcode("v_{$column_key}", array($this,'new_column'));
        }
        /* ADMIN  PART */
        //Show if Exists
        ...
        add_action('init',array($this,'initialise_vibe_builder'),2);

        
    }


    function initialise_vibe_builder(){
      if(function_exists('vibe_get_option')){
          if(vibe_get_option('vibe_page_builder')){
            add_action( 'admin_enqueue_scripts', array($this,'v_scripts_styles'), 10, 1 );
            add_action('admin_init',array($this,'v_new_modules_init'));
    ...
    function add_vibe_editor(){
        add_meta_box( 'vibe-editor', __( 'Page Builder', 'wplms' ),array($this,'vibe_layout_editor') , 'page', 'normal', 'high' );
    }
...
function custom_post_carousel($atts, $content = null) {
    if(function_exists('vibe_get_option') && vibe_get_option('offload_scripts')){
         wp_enqueue_script('flickity',plugins_url('../../../../assets/js/flickity.pkgd.min.js',__FILE__),array(),WPLMS_VERSION,true);
          wp_enqueue_style('flickity',plugins_url('../../../../assets/css/flickity.min.css',__FILE__),array(),WPLMS_VERSION);
          wp_enqueue_script( 'vibecorousel-js', WPLMS_PLUGIN_INCLUDES_URL . '/vibe-shortcodes/js/carousel.js',array('flickity'),WPLMS_PLUGIN_VERSION,true);
          wp_enqueue_style( 'shortcodess-css', WPLMS_PLUGIN_INCLUDES_URL.'/vibe-shortcodes/css/shortcodes.css',array(),WPLMS_PLUGIN_VERSION);
  ...
  if(!isset($atts['post_ids']) || strlen($atts['post_ids']) < 2){
        ...
        $output .= '
          ...
          #'.$rand.' ul.slides div>li{margin:0 1.5rem;}
         </style>
        ';
        $min = 1;$max=4;
        if(!empty($atts['carousel_min'])){
          $min = $atts['carousel_min'];
        }
        if(!empty($atts['carousel_max'])){
          $max = $atts['carousel_max'];
          $max = ceil(100/$max);
        ...
        if(!empty($atts['carousel_rows']) && $atts['carousel_rows'] > 1 && $row_logic<$atts['carousel_number']){

          
          if($row_logic%$atts['carousel_rows'] == 0){
            $output .= '<li><div class="carousel_rows">';  
          }
          

          //for($i=0;$i<$atts['carousel_rows'];$i++){
          ...
          //}
          if($row_logic%$atts['carousel_rows'] == ($atts['carousel_rows'] -1)){
            $output .= '</div></li>';
          }
          $row_logic ++;
  ...
  function wplms_custom_post_carousel($atts, $content = null) {
	...
	if(!isset($atts['post_ids']) || strlen($atts['post_ids']) < 2){
        ...
        if(!empty($atts['course_type'])){
          if($atts['course_type'] == 'mycourses'){
            $user_id = bp_displayed_user_id();
            $courses = bp_course_get_user_courses($user_id);  
            $query_args['post__in'] = array_unique($courses);

            $init = WPLMS_4_Filters::init();
            $init->featured_courses = array_unique($courses);
          }
          if($atts['course_type'] == 'instructing_courses'){
             $user_id = bp_displayed_user_id();
        ...
        $more= '';
        if(isset($atts['show_more']) && $atts['show_more']) {
            $more = ' <a href="'.$atts['more_link'].'" class="heading_more"><span class="vicon vicon-plus"></span></a>';
        }
        $noheading='';
        
        if($atts['show_title']){
            $output .='<h3 class="heading"><span>'.$atts['title'].'</span></h3>'.$more;
            $noheading = 'has_heading';
        }
        

        $class='flickity_slides';
        
        if(empty($rand))
          $rand = 'carousel_'.rand(0,999);
        ...
        $output .= '<div id="'.$rand.'" class="vibe_flickity_carousel loading '.$noheading.' '.((isset($atts['show_more']) && $atts['show_more'])?'more_heading':'').'">
  	            <div class="'.$class.'">';
  	     $links='';
         $excerpt='';
         $thumb='';
        ...
        if(!empty($atts['carousel_rows']) && $atts['carousel_rows'] > 1 && $row_logic<$atts['carousel_number']){
          ...
          if($row_logic%$atts['carousel_rows'] == 0){
            $output .= '<div class="carousel-cell"><div class="carousel_rows">';  
          }
        ...
        <script>
          window.addEventListener('DOMContentLoaded', function(event){
            ...
            var <?php echo $rand; ?> = new Flickity( document.querySelector('#<?php echo $rand; ?> .flickity_slides'), {
              initialIndex: 0,
              imagesLoaded: true,
              wrapAround: true,
              prevNextButtons: <?php echo (empty($atts['show_controls'])?'false':'true'); ?>,
              pageDots: <?php echo (empty($atts['show_controlnav'])?'false':'true'); ?>,
              groupCells: true,
              autoPlay: <?php echo empty($atts['auto_slide'])?'false':'true'; ?>,
              on: {
                ready: function() {
                  document.querySelector('#<?php echo $rand; ?>').classList.remove('loading');
                },
        ...
        <style type="text/css">
          .vibe_flickity_carousel{
            padding: 0 -<?php echo $atts['carousel_space']/2;?>px;
          }
          .vibe_flickity_carousel .carousel-cell{
            float: left;
            width: <?php echo round(100/$atts['carousel_max']); ?>%;
          }.vibe_flickity_carousel .carousel-cell .block{
            margin: 0 <?php echo $atts['carousel_space']/2;?>px;
          }
          .flickity-viewport {
          ...
          @media (max-width:768px){
            .vibe_flickity_carousel .carousel-cell{
              width: <?php echo round(100/$atts['carousel_min']); ?>%;
            }
          }
        </style>
        <?php
    ...
    function taxonomy_carousel($atts,$content=null){
        ...
        $output .= '
          ...
          @media (min-width:481px)  { #'.$rand.' ul.slides div>li{width:100%;}}
        ...
        $output .= '<div id="'.$rand.'" class="vibe_carousel flexslider loading '.(($atts['carousel_max']==1)?'onecol':'').' '.$noheading.' '.((isset($atts['show_more']) && $atts['show_more'])?'more_heading':'').'" data-directionnav="'.(Empty($atts['show_controls'])?0:$atts['show_controls']).'" data-controlnav="'.(empty($atts['show_controlnav'])?0:$atts['show_controlnav']).'" data-block-width="'.$atts['column_width'].'" data-block-max="'.$atts['carousel_max'].'" data-block-min="'.$atts['carousel_min'].'" data-block-move="'.(empty($atts['carousel_move'])?$atts['carousel_min']:$atts['carousel_move']).'" data-autoslide="'.$atts['auto_slide'].'">
            ...
            if(!empty($atts['term_slugs'])){
                ...
                if(!empty($slugs))
                    $args = array('taxonomy'=>'course-cat','include'=>$slugs,'number'=>$atts['carousel_number']);
            }else{
               $args = array('taxonomy'=>'course-cat','number'=>$atts['carousel_number']);
            }
            if(!empty($atts['hide_child'])){
              $args['parent'] = 0;
            ...
            if(!empty($terms)){
                ...
                for($i=0;$i<count($terms); $i++){ 
                    ...
                    if(!empty($atts['carousel_rows']) && $atts['carousel_rows'] > 1 && !empty($terms[$i])){
                      if($row_logic%$atts['carousel_rows'] == 0){
                        $output .= '<li><div class="carousel_rows">';
                      }
                      //for($k=$i;$k < ($i+$atts['carousel_rows']);$k++){

                        $thumbnail_id = get_term_meta( $terms[$i]->term_id, 'course_cat_thumbnail_id', true );
                        if ( $thumbnail_id ) {
                        ...
                        $output .= '<div class="carousel_row">';
                        $output .= '<a href="'.get_term_link($terms[$i]).'" class="term_block"><img src="'.$image.'"><strong class="term_name"><span>'.$terms[$i]->name.'</span></strong></a>';  
                         $output .= '</div>';
                      ...
                      if(!empty($terms[$i])){
                        ...
                        $output .= '<li>';
                        $output .= '<a href="'.get_term_link($term).'" class="term_block"><img src="'.$image.'"><strong class="term_name"><span>'.$term->name.'</span></strong></a>';  
                        $output .= '</li>';
                      }
                    }  
                }    
            }else{
                $output.='<li><div class="message">'._x('No terms found.','no user found in selection in page builder member block','wplms').'</div></li>';
            }
        ...
        $output .= '<div id="'.$rand.'" class="vibe_carousel flexslider loading '.(($atts['carousel_max']==1)?'onecol':'').' '.$noheading.' '.((isset($atts['show_more']) && $atts['show_more'])?'more_heading':'').'" data-directionnav="'.(empty($atts['show_controls'])?0:$atts['show_controls']).'" data-controlnav="'.(empty($atts['show_controlnav'])?0:$atts['show_controlnav']).'" data-block-width="'.$atts['column_width'].'" data-block-max="'.$atts['carousel_max'].'" data-block-min="'.$atts['carousel_min'].'" data-block-move="'.(empty($atts['carousel_move'])?$atts['carousel_min']:$atts['carousel_move']).'" data-autoslide="'.$atts['auto_slide'].'">
            ...
            if(!empty($terms)){
                foreach($terms as $term){ 
                    ...
                    $output .= '<li class="carousel-cell"><a href="'.get_term_link($term).'" class="term_block"><img src="'.$image.'"><strong class="term_name"><span>'.$term->name.'</span></strong></a></li>';
                }    
            }else{
                $output.='<li><div class="message">'._x('No terms found.','no user found in selection in page builder member block','wplms').'</div></li>';
            }
        ...
        $output .= '<div id="'.$rand.'" class="vibe_carousel flexslider loading '.(($atts['carousel_max']==1)?'onecol':'').' '.$noheading.' '.((isset($atts['show_more']) && $atts['show_more'])?'more_heading':'').'" data-directionnav="'.$atts['show_controls'].'" data-controlnav="'.$atts['show_controlnav'].'" data-block-width="'.$atts['column_width'].'" data-block-max="'.$atts['carousel_max'].'" data-block-min="'.$atts['carousel_min'].'" data-block-move="'.(empty($atts['carousel_move'])?$atts['carousel_min']:$atts['carousel_move']).'" data-autoslide="'.$atts['auto_slide'].'">
            ...
            if(!empty($user_ids)){
                ...
                for($i=0;$i<count($user_ids); $i++){ 
                    ...
                    if(!empty($atts['carousel_rows']) && $atts['carousel_rows'] > 1 && !empty($user_ids[$i])){
                      ...
                      $output .= '<div class="carousel_row">';
                      $output .= vibe_member_block($user_ids[$i],$atts['style'],$field_names,$atts['column_width'],$atts['carousel_link']);;  
                      $output .= '</div>';  
                ...
                $output.='<li><div class="message">'._x('No user found.','no user found in selection in page builder member block','wplms').'</div></li>';
            }
    ...
    function member_grid($atts,$content=null){
        ...
        /*
            ...
            'exclude_member_ids' => array(
              'title' => __('Exclude Specific Member Ids (comma separated)', 'wplms'),
              'type' => 'text',
              'std'=>''
            ),             
            'style' => array(
                    'title' => __('Display Style', 'wplms'),
                'type' => 'radio_images',
            ...
            'block_size' => array(
               'title' => __('Block Size (in px) Or number of blocks in 1 row based on smallest block size ', 'wplms'),
               'type' => 'text',
               'std' => 3
            ), 

            'gutter' => array(
              'title' => __('Spacing between Columns (in px)', 'wplms'),
              'type' => 'text',
              'std' => '0'
            ),             
            'grid_number' => array(
              'title' => __('Total Number of Blocks in Grid', 'wplms'),
              'type' => 'text',
              '
          */
    ...
    function custom_post_filterable($atts, $content = null) {
        ...
        wp_enqueue_script( 'isotope', plugins_url('../../../../assets/js/isotope.pkgd.min.js',__FILE__),array(),WPLMS_PLUGIN_VERSION);
        
        if(is_wplms_4_0()){
            wp_enqueue_script('v4shortcodes',plugins_url('../../../../assets/js/shortcodes.js',__FILE__),array(),WPLMS_PLUGIN_VERSION,true);
            wp_enqueue_style('v4shortcodes',plugins_url('../../../../assets/js/shortcodes.css',__FILE__),array(),WPLMS_PLUGIN_VERSION);
        }else{
       ...
       $output .= "<div {$attributes['class']}{$attributes['inline_styles']}>";
        ...
        if($atts['show_all'])                
        $output .='<li class="active"><a href="javascript:void();" data-filter="*" class="all">'.__('All','wplms').'</a></li>';
	...
	if(!isset($atts['post_ids']) || strlen($atts['post_ids']) < 2){
        ...
        if(!empty($atts['course_type'])){
          ...
          if($atts['course_type'] == 'mycourses'){
            ...
            $user_id = apply_filters('wplms_vibe_grid_user_id',$user_id,$atts, $query_args);
            $courses = bp_course_get_user_courses($user_id); 
            if(!empty($courses)){
              $query_args['post__in'] = array_unique($courses);
              $init = WPLMS_4_Filters::init();
          ...
          if($atts['course_type'] == 'instructing_courses'){
            ...
            if(!empty($courses)){
              ...
              foreach ($courses as $key => $cc) {
                ...
                if(!empty($cc['course_id'])){
                  if(!in_array($cc['course_id'], $_Courses)){

                    $_Courses[] = $cc['course_id'];
                  }
                }else{
                  if(!in_array($cc['value'], $_Courses)){
        ...
        if($atts['grid_search']){
          $output .='<div class="grid_search">
            <form method="get">
              <input type="text" placeholder="'.__('Hit enter to search','wplms').'" name="s" value="'.(empty($s)?'':$s).'" />
              <span class="vicon vicon-search"></span>
            </form>
          </div>';
        }
        $output .= '<div class="vibe_grid '.$infinite.' '.$masonry.'" '.$rel.'><div class="wp_query_args" style="display:none;" data-max-pages="'.$wp_query->max_num_pages.'">'.  json_encode($atts).'</div>';
  	 if(empty($masonry)){
        ...
        $output .= '<ul class="grid row '.$masonry.'" '.$istyle.'>';
        ...
        if(isset($atts['infinite']) && $atts['infinite']){
            $output .= '<div class="load_grid"><span>'.__('Loading..','wplms').'</span></div>
                        <div class="end_grid"><span>'.__('No more to load','wplms').'</span></div>';
        }
        $output .="</div>";
        if(isset($atts['pagination']) && $atts['pagination']){
        ob_start();
...
// dialog box columns
    ...
    if ( floatval(get_bloginfo('version')) >= 3.9){
      ...
      wp_enqueue_script( 'v_admin_js',plugins_url( 'js/v_admin_old.js' , __FILE__ ), array('jquery','jquery-ui-core','jquery-ui-sortable','jquery-ui-draggable','jquery-ui-droppable','jquery-ui-resizable'), '1.0' );
		...
		wp_localize_script( 'v_admin_js', 'v_options', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ), 'load_nonce' => wp_create_nonce( 'load_nonce' ), 'confirm_message' => __('Confirm Delete?', 'wplms'), 'confirm_message_yes' => __('Yes', 'wplms'), 'confirm_message_no' => __('No', 'wplms'), 'saving_text' => __('Saving...', 'wplms'), 'saved_text' => __('Saved.', 'wplms') ) );
	}
    ...
    function vibe_builder_enqueue_shortcode_scripts() {
      ...
      if(!empty($post) && !empty($post->ID)){
        ...
        $flag = apply_filters('wplms_enqueue_images_loaded_isotopes_page_builder',0,$post);
        if ( ((isset($builder_layout) && !empty($builder_layout['layout_shortcode']) && has_shortcode($builder_layout['layout_shortcode'],'v_grid') ) || has_shortcode($post->post_content,'v_grid')) || $flag) { 
    ...
    function show_builder_layout( $content ){
      ...
      $builder_enable = empty($post)?'':get_post_meta( $post->ID, '_enable_builder', true );
      $builder_layout = empty($post)?'':get_post_meta( $post->ID, '_builder_settings', true );
      $add_content = empty($post)?'':get_post_meta( $post->ID, '_add_content', true );
    ...
    /*
      LEGACY
    */
    function vibe_editor_save_details(){
...
/* ===== Declaring the Modules =======  */                
    $this->v_modules['carousel'] = array(
          'name' => __('Carousels/Rotating Blocks', 'wplms'),
          'options' => array(

            'title' => array(
              'title' => __('Title/Heading', 'wplms'),
              'type' => 'text',
              'std' => __('Heading', 'wplms')
            ), 

            'show_title' => array(
              'title' => __('Show Title', 'wplms'),
              'type' => 'select_yesno',
            ...
            'more_link' => array(
              'title' => __('More Link (User redirected to this page on click)', 'wplms'),
              'type' => 'text',
              'std' => ''
            ), 

            'show_controls' => array(
              'title' => __('Show Direction arrows', 'wplms'),
              'type' => 'select_yesno',
              'options' => array(0=>'No',1=>'Yes'),
              'std' => 1
            ), 
            ...
            'show_controlnav' => array(
              'title' => __('Show Control dots', 'wplms'),
              'type' => 'select_yesno',
              'options' => array(0=>'No',1=>'Yes'),
              'std' => 0
            ),
            ...
            'post_type' => array(
              'title' => __('Enter Post Type<br /><span style="font-size:11px;">(Select Post Type from Posts/Courses/Clients/Products ...)</span>', 'wplms'),
              'type' => 'select',
            ...
            'taxonomy' => array(
              'title' => __('Enter Taxonomy Slug (optional)<br /><span style="font-size:11px;">(A "Taxonomy" is a grouping mechanism for posts. Like Category for Posts, Tags for Posts, Portfolio Type for Portfolio etc.. <a href="http://codex.wordpress.org/Taxonomies">more</a>)</span> ', 'wplms'),
              'type' => 'text',
              'std' => ''
            ), 

        'term' => array(
        'title' => __('Enter Taxonomy Term Slug (optional, only if above is selected, comma separated for multiple terms): ', 'wplms'),
        'type' => 'text',
        'std' => ''
      ),   
            'post_ids' => array(
          'title' => __('Or Enter Specific Post Ids', 'wplms'),
          'type' => 'text',
                    'std'=>''
      ),   
            'course_style' => array(
              'title' => __('Course Types [Only for Post type = Course]', 'wplms'),
              'type' => 'select',
              'options' => array(
            ...
            'featured_style' => array(
              'title' => __('Carousel/Rotating Block Style', 'wplms'),
              'type' => 'radio_images',
              'options' => $v_thumb_styles,
              'std' => __('excerpt', 'wplms')
            ),
            'auto_slide' => array(
              'title' => __('Auto slide/rotate', 'wplms'),
              'type' => 'select_yesno',
              'options' => array(0=>'No',1=>'Yes'),
              'std' => 1,
            ),            
            'column_width' => array(
              'title' => __('Width each crousel block', 'wplms'),
              'type' => 'text',
              'std' => __('268', 'wplms')
            ), 
            'carousel_max' => array(
              'title' => __('Maximum Number of blocks in One screen', 'wplms'),
              'type' => 'text',
              'std' => __('4', 'wplms')
            ), 
            'carousel_min' => array(
              'title' => __('Minimum Number of blocks in one Screen', 'wplms'),
              'type' => 'text',
              'std' => __('2', 'wplms')
            ),
            'carousel_move' => array(
              'title' => __('Move blocks in one slide', 'wplms'),
              'type' => 'text',
              'std' => 1,
            ),
            'carousel_rows' => array(
              'title' => __('Carousel Rows', 'wplms'),
              'type' => 'text',
              'std' => __('1', 'wplms')
            ),           
            'carousel_number' => array(
              'title' => __('Total Number of Blocks', 'wplms'),
              'type' => 'text',
              'std' => __('6', 'wplms')
            ), 
        
            'carousel_excerpt_length' => array(
              'title' => __('Excerpt Length in Block (in characters)', 'wplms'),
              'type' => 'text',
              'std' => __('100', 'wplms')
            ),  
            'carousel_link' => array(
              'title' => __('Show Link button on image hover', 'wplms'),
              'type' => 'select_yesno',
              'options' => array(0=>'No',1=>'Yes'),
              'std' => __(1, 'wplms')
            ), 
            'advanced_settings' => array(
                'title' => __('Show Advanced settings', 'wplms'),
                'type' => 'divider',
                'std' => 3
            ),             
            'css_class' => array(
               'title' => __('* Custom Class name (Add Custom Class to this Block)', 'wplms'),
               'type' => 'text'
            ),
            'container_css' => array(
                'title' => __('* Class for on containing Layout column', 'wplms'),
                'type' => 'text'
            ),
            'custom_css' => array(
                 'title' => __('* Add Custom CSS (Use <strong>.</strong> for class name, <strong>:hover</strong> for hover styles etc..)', 'wplms'),
                 'type' => 'textarea'
                ),             
      ),
    );
    ...
    /* ====== Filterable ===== */
        ...
        $this->v_modules['filterable'] = array(
          'name' => __('Filterable Posts', 'wplms'),
          'options' => array(
                       
            'title' => array(
                'title' => __('Filterable Block Title', 'wplms'),
                'type' => 'text',
                'std' => __('Heading', 'wplms')
              ), 
            'show_title' => array(
            ...
            'post_type' => array(
              'title' => __('Select a Post Type', 'wplms'),
              'type' => 'select',
              'options' => $v_post_types,
              'std' => __('post', 'wplms')
            ),    
            'taxonomy' => array(
              'title' => __('Enter relevant Taxonomy name used for Filter buttons (example : course-cat,event-type..)', 'wplms'),
              'type' => 'text',
              'std' => ''
            ),
            'exclude_terms' => array(
              'title' => __('Enter terms (comma separated term slugs) to be "excluded" in filters', 'wplms'),
              'type' => 'text',
              'std' => ''
            ),
            'course_style' => array(
            ...
            'featured_style' => array(
              'title' => __('Featured Media Block Style', 'wplms'),
              'type' => 'radio_images',
              'options' => $v_thumb_styles,
              'std' => __('excerpt', 'wplms')
            ), 
            'show_all' => array(
              'title' => __('Show All link', 'wplms'),
              'type' => 'select_yesno',
              'options' => array(0=>'No',1=>'Yes'),
              'std' => __(1, 'wplms')
            ),   
            'column_width' => array(
              'title' => __('Column Width (in px)', 'wplms'),
              'type' => 'text',
              'std' => '200'
            ),           
            'filterable_excerpt_length' => array(
              'title' => __('Excerpt Length (in characters)', 'wplms'),
              'type' => 'text',
              'std' => __('100', 'wplms')
            ),              
            ...
            'show_pagination' => array(
              'title' => __('Show Pagination', 'wplms'),
              'type' => 'select_yesno',
              'options' => array(0=>'No',1=>'Yes'),
              'std' => __(1, 'wplms')
            ),  
            ...
            'filterable_link' => array(
              'title' => __('Show Link [Links to Post]', 'wplms'),
              'type' => 'select_yesno',
              'options' => array(0=>'No',1=>'Yes'),
              'std' => __(1, 'wplms')
            ), 
    ...
    /* ===== Grid =======  */                
        ...
        $this->v_modules['grid'] = array(
          'name' => __('Post Grid', 'wplms'),
          'options' => array(
                       
            'title' => array(
              'title' => __('Grid Title', 'wplms'),
              'type' => 'text',
              'std' => __('Heading', 'wplms')
            ), 
            'show_title' => array(
            ...
            'post_type' => array(
              'title' => __('Custom Post Type', 'wplms'),
              'type' => 'select',
              'options' => $v_post_types,
              'std' => __('post', 'wplms')
            ),
            ...
            'term' => array(
              'title' => __('Enter Taxonomy Term Slugs (optional, only if above is selected, comma separated if multiple): ', 'wplms'),
              'type' => 'text',
              'std' => ''
            ),   

            'post_ids' => array(
              'title' => __('Or Enter Specific Post Ids (comma separated)', 'wplms'),
              'type' => 'text',
              'std'=>''
            ),             
            'course_style' => array(
            ...
            'masonry' => array(
              'title' => __('Grid Masonry Layout', 'wplms'),
              'type' => 'select_yesno',
              'options' => array(0=>'No',1=>'Yes'),
              'std' => __(0, 'wplms')
            ),     
            ...
            'column_width' => array(
              'title' => __('Masonry Grid Column Width(in px)', 'wplms'),
              'type' => 'text',
              'std' => '200'
            ), 
            'gutter' => array(
            ...
            'infinite' => array(
              'title' => __('Infinite Scroll', 'wplms'),
              'type' => 'select_yesno',
              'options' => array(0=>'No',1=>'Yes'),
              'std' => __(1, 'wplms')
            ), 
            ...
            'pagination' => array(
              'title' => __('Enable Pagination (If infinite scroll is off)', 'wplms'),
              'type' => 'select_yesno',
              'options' => array(0=>'No',1=>'Yes'),
              'std' => __(1, 'wplms')
            ),            
    ...
    /* ====== Editor ===== */                
      $this->v_modules['text_block'] = array(
          'name' => __('WP Editor', 'wplms'),
          'options' => array(
                'title' => array(
                      'title' => __('Reference Title', 'wplms'),
                      'type' => 'text',
                      'std' => __('Content', 'wplms')
                             ), 
                'text_block_content' => array(
                  'title' => __('Content', 'wplms'),
                  'type' => 'wp_editor',
                  'is_content' => true
                ),
                'advanced_settings' => array(
                ...
                'animation_effect' => array(
                   'title' => __('* On-Load CSS3 Animation effect on the block (<a href="http://vibethemes.com/documentation/wplms/knowledge-base/css3-transitions-in-page-builder/" target="_blank">more</a>)', 'wplms'),
                   'type' => 'select',
    ...
    /* ====== Parallax ===== */                
        $this->v_modules['parallax_block'] = array(
          'name' => __('Parallax Content', 'wplms'),
          'options' => array(
                'title' => array(
                      'title' => __('Reference Title', 'wplms'),
                      'type' => 'text',
                      'std' => __('Parallax Title', 'wplms')
                    ), 
                  'text_block_content' => array(
                    'title' => __('Content', 'wplms'),
                    'type' => 'wp_editor',
                  ...
                  'bg_image' => array(
                        'title' => __('Upload Parallax Background image', 'wplms').'<a href="http://vibethemes.com/documentation/wplms/knowledge-base/adding-or-uploading-image-for-badge-parallax-logo-notworking/" target="_blank" style="text-decoration:none;color:#666">&nbsp;<span class="dashicons dashicons-warning"></span></a>',
                        'type' => 'upload',
                        'std' => ''
                    ),  
                 'rev' => array(
                      'title' => __('Background Effect', 'wplms'),
                      'type' => 'select',
                      'options' => array(
                            ''=>__('Image Scrolls with scroll','wplms'),
                            1=>__('Image Static with Scroll','wplms'),
                            2=>__('Full Parallax Static with Scroll','wplms')
                            ),
                      'std' => ''
                    ),  
                  'height' => array(
                      'title' => __('Parallax Block Height (in px)', 'wplms'),
                      'type' => 'text',
                      'std' => '200'
                    ), 
                   'scroll' => array(
                        'title' => __('Parallax value (Scroll senstivity, lower value means higher scroll)', 'wplms'),
                        'type' => 'text',
                        'std' => '2'
                    ),
                    'adjust' => array(
                      'title' => __('Adjust background (in px)', 'wplms'),
                      'type' => 'text',
                      'std' => '0'
                    ),
                    'padding_top' => array(
                      'title' => __('Top Padding(in px)', 'wplms'),
                      'type' => 'text',
                      'std' => '0'
                    ),
                    'padding_bottom' => array(
                      'title' => __('Bottom Padding(in px)', 'wplms'),
                      'type' => 'text',
                      'std' => '0'
                    ),           
                   'advanced_settings' => array(
    ...
    $this->v_modules['featured_block'] = array(
          'name' => __('Featured Content', 'wplms'),
          'options' => array(
                  'title' => array(
                      'title' => __('Reference Title', 'wplms'),
                      'type' => 'text',
                      'std' => __('reference Title', 'wplms')
                    ), 
                  'style' => array(
                    'title' => __('Content Style', 'wplms'),
                    'type' => 'radio_images',
                    'options' => $v_content_styles,
                    'std'=>''
                  ), 
                  'image' => array(
                        'title' => __('Upload Image (Optional)', 'wplms'),
                        'type' => 'upload',
                        'std' => ''
                    ),
                  'icon' => array(
                        'title' => __('Select Icon (Optional)', 'wplms'),
                        'type' => 'icon',
                        'std' => ''
                  ), 
                  'text_block_content' => array(
                  ...
                  'style_settings' => array(
                      'title' => __('Style settings', 'wplms'),
                      'type' => 'divider',
                      'std' => 9
                    ),  
                  'icon_size' => array(
                    'title' => __('Icon Size/Image Width (in px)', 'wplms'),
                    'type' => 'text',
                    'std' => '0'
                  ),
                  'icon_color' => array(
                    'title' => __('Icon Color', 'wplms'),
                    'type' => 'color',
                    'std' => '#009dd8'
                  ), 
                 'padding_top' => array(
                    'title' => __('Top Padding / Top Spacing (in px)', 'wplms'),
                    'type' => 'text',
                    'std' => '0'
                  ),
                  'padding_bottom' => array(
                    'title' => __('Bottom Padding / Bottom Spacing (in px)', 'wplms'),
                    'type' => 'text',
                    'std' => '0'
                  ), 
                  'background_color' => array(
                    'title' => __('Background Color', 'wplms'),
                    'type' => 'color',
                    'std' => '#FFFFFF'
                  ), 
                  'text_color' => array(
                    'title' => __('Text Color', 'wplms'),
                    'type' => 'color',
                    'std' => '#666666'
                  ),
                  'border' => array(
                      'title' => __('Border', 'wplms'),
                      'type' => 'select',
                      'options' => array(
                            ''=>__('No','wplms'),
                            '1'=>__('Yes','wplms'),
                  ...
                  'border_width' => array(
                    'title' => __('Border width', 'wplms'),
                    'type' => 'text',
                    'std' => '1'
                  ), 
                  'border_color' => array(
                    'title' => __('border Color', 'wplms'),
                    'type' => 'color',
                    'std' => '#EEEEEE'
                  ),         
                   'advanced_settings' => array(
...
$this->v_modules['member_carousel'] = array(
                'name' => __('Member Carousels/Rotating Blocks', 'wplms'),
                'options' => array(
            ...
            'member_type' => array(
                        'title' => __('Select member type', 'wplms'),
                        'type' => 'select',
                        'options' => apply_filters('vibe_editor_member_types',array('all'=>_x('All','Select option in Member carousel','wplms'),'student'=>_x('Student','Select option in Member carousel','wplms'),'instructor'=>_x('Instructor','Select option in Member carousel','wplms'))),
                        'std' => '',
                    ),
                  
            'member_ids' => array(
                        'title' => __('Or Enter Specific Member Ids', 'wplms'),
                        'type' => 'text',
                        'std'=>''
            ),  
            'profile_fields' => array(
                        'title' => __('Enter Profile fields (comma separated field "names")', 'wplms'),
                        'type' => 'text',
                        'std' => ''
            ),  
            'style' => array(
...
$this->v_modules['member_grid'] = array(
          'name' => __('Member Grid', 'wplms'),
          'options' => array(
            ...
            'member_ids' => array(
              'title' => __('Only Specific Member Ids (comma separated)', 'wplms'),
              'type' => 'text',
              'std'=>''
            ),             
            'style' => array(
...
/* ====== RevSlider ===== */
    ...
    $this->v_modules['revslider'] = array(
          'name' => __('Revolution Slider', 'wplms'),
          'options' => array(
                 'alias' => array(
                         'title' => __('Select Slider', 'wplms'),
                         'type' => 'select',
                         'options' => $revsliders
                  ),  
                  'advanced_settings' => array(
    ...
    $this->v_modules['layerslider'] = array(
          'name' => __('Layer Slider', 'wplms'),
          'options' => array(
            'id' => array(
                    'title' => __('Select Slider', 'wplms'),
                    'type' => 'select',
    ...
    $this->v_modules['taxonomy_carousel'] = array(
        'name' => __('Course Category Carousel', 'wplms'),
        'options' => array(
            'title' => array(
                'title' => __('Title/Heading', 'wplms'),
                'type' => 'text',
            ...
            'term_slugs' => array(
                'title' => __('Include Term Slugs (optional, comma separated)', 'wplms'),
                'type' => 'text',
                'std' => '',
            ),
             'orderby' => array(
                ...
                'type' => 'select',
                'type' => 'select',
                'options' => array('name'=>__('Alphabetical', 'wplms'),'description'=>__('Description', 'wplms'),'meta_value_num'=>__('Custom Order','wplms')),
                'std' => 1
            ), 
            'order' => array(
                'title' => __('Show Direction arrows', 'wplms'),
                'type' => 'select',
                'options' => array('DESC'=>__('Decending', 'wplms'),'ASC'=>__('Ascending', 'wplms')),
                'std' => 1
            ), 
            'show_controls' => array(
                'title' => __('Show Direction arrows', 'wplms'),
            ...
            'custom_css' => array(
                ...
                //Sidebars
    $this->v_modules['widget_area'] = array(
      'name' => __('Sidebar', 'wplms'),
      'options' => array(
            'area' => array(
                  'title' => __('Select a Sidebar', 'wplms'),
                  'type' => 'select',
                  'options' => $v_widget_areas,
                  'std' => __('MainSidebar', 'wplms')
                ),
              'advanced_settings' => array(
                  'title' => __('Show Advanced settings', 'wplms'),
                  'type' => 'divider',
    ...
    $this->v_modules['slider'] = array(
          'name' => __('FlexSlider', 'wplms'),
          'options' => array(
              'title' => array(
                    'title' => __('Slider ID (for reference & Css)', 'wplms'),
                    'type' => 'text',
                    'std' => 'FlexSlider'
                ),
                'slide_style' => array(
                      'title' => __('Slide Style', 'wplms'),
                      'type' => 'radio_images',
                      'options'=>array(
                  ...
                  'animation' => array(
                      'title' => __('Animation Effect', 'wplms'),
                      'type' => 'select',
                      'options' => array( 'fade'=>__('fade', 'wplms'),'slide'=> __('slide', 'wplms') ),
                      'std' => 'fade'
                    ),
                                    
                  'slider_settings' => array(
                      'title' => __('Slider settings', 'wplms'),
                      'type' => 'divider',
                      'std' => 12
                  ),
                  'auto_slide' => array(
                    'title' => __('Auto slide Images', 'wplms'),
                    'type' => 'select_yesno',
                    'options' => array(0=>'No',1=>'Yes'),
                    'std' => __(1, 'wplms')
                  ),
                  'loop' => array(
                      'title' => __('Loop Slides', 'wplms'),
                      'type' => 'select_yesno',
                      'options' => array(0=>'No',1=>'Yes'),
                      'std' => __(1, 'wplms')
                    ),
                    'randomize' => array(
                      'title' => __('Randomize Slides', 'wplms'),
                      'type' => 'select_yesno',
                      'options' => array(0=>'No',1=>'Yes'),
                      'std' => __(1, 'wplms')
                    ),
                    'show_directionnav' => array(
                          'title' => __('Show Slider Direction arrows', 'wplms'),
                          'type' => 'select_yesno',
                          'options' => array(0=>'No',1=>'Yes'),
                          'std' => __(1, 'wplms')
                        ),
                    'show_controlnav' => array(
                        'title' => __('Show Slider Control buttons', 'wplms'),
                        'type' => 'select_yesno',
                        'options' => array(0=>'No',1=>'Yes'),
                        'std' => __(1, 'wplms')
                      ),
            'animation_duration' => array(
              'title' => __('Animation Duration (in ms)', 'wplms'),
              'type' => 'text',
              'std' => '600'
            ),
            
            'auto_speed' => array(
              'title' => __('Auto Animation Speed (in ms)', 'wplms'),
              'type' => 'text',
              'std' => '7000'
            ),
            'pause_on_hover' => array(
              'title' => __('Pause Slider On Hover', 'wplms'),
              'type' => 'select_yesno',
              'options' => array(0=>'No',1=>'Yes'),
              'std' => __(1, 'wplms')
            ),
    ...
    $this->v_modules = apply_filters( 'v_modules', $this->v_modules );
    
    $this->v_columns['1_2'] = array( 'name' => __('1/2 Column', 'wplms') );
    $this->v_columns['1_3'] = array( 'name' => __('1/3 Column', 'wplms') );
    $this->v_columns['1_4'] = array( 'name' => __('1/4 Column', 'wplms') );
        $this->v_columns['1_4_2'] = array( 'name' => __('1/4 Column & 1/2 Column', 'wplms') );
    $this->v_columns['2_3'] = array( 'name' => __('2/3 Column', 'wplms') );
    $this->v_columns['3_4'] = array( 'name' => __('3/4 Column', 'wplms') );
    $this->v_columns['resizable'] = array( 'name' => __('Full-Width Resizable Column', 'wplms') );
    $this->v_columns['stripe_container'] = array( 'name' => __('FullScreen Stripe with Container', 'wplms') );
        $this->v_columns['stripe'] = array( 'name' => __('FullScreen Stripe', 'wplms') );
                
    $this->v_columns = apply_filters( 'this->v_columns', $this->v_columns );
    $this->v_sample_layouts='';
    $this->v_sample_layouts = get_option('vibe_builder_sample_layouts');
                if(is_string($this->v_sample_layouts))
                    $this->v_sample_layouts = unserialize($this->v_sample_layouts);
                
    foreach( $this->v_columns as $v_column_key => $v_column ){
  ...
  ?>
    ...
    <div id="page_builder">
      <div id="vibe_editor_controls" class="clearfix">
        <a href="#" class="add_element add_column"><span><i class="dashicons dashicons-screenoptions"></i> <?php _e('COLUMNS', 'wplms'); ?></span></a>
        <a href="#" class="add_element add_module"><span><i class="dashicons dashicons-welcome-widgets-menus"></i> <?php _e('CONTENT', 'wplms'); ?></span></a>
        <a href="#" class="add_element add_sample_layout"><span><i class="dashicons dashicons-feedback"></i> <?php _e('SAVED LAYOUTS', 'wplms'); ?></span></a>
      </div> <!-- #vibe_editor_controls -->
      
      <div id="modules">
        <?php

          foreach ( $this->v_modules as $module_key => $module_settings ){
          ...
          if(is_array($this->v_columns))
          foreach ( $this->v_columns as $column_key => $column_settings ){
          ...
          if(is_array($this->v_sample_layouts))
          foreach ( $this->v_sample_layouts as $layout_key => $layout_settings ){
      ...
      <div id="layout_container">
        ...
        <div id="v_helper"<?php echo $v_helper_class; ?>><?php esc_html_e('Drag & Drop Layout Columns and then Drag & Drop Content Blocks to each column', 'wplms'); ?></div>
      ...
      <div style="display: none;">
        ...
        ?>
                ...
                <div class="overlay">
                                <label><?php _e('Enter name of Sample Layout','wplms'); ?></label><input type="text" class="text" id="new_sample_layout_name" name="new_sample_layout_name" data-id="<?php global $post; echo $post->ID;?>"/>
                                <a id="save_new_sample_layout" class="vibe-button-save-new-layout"><?php _e('Save Layout', 'wplms') ?></a>
                                <span class="remove"></span>
                </div>
    <div id="v_ajax_save">
      <img src="<?php echo plugins_url('images/loading.gif',__FILE__ ); ?>" alt="loading" id="loading" />
      <span><?php esc_html_e( 'Saving...', 'wplms' ); ?></span>
    </div>
    
    <?php
      echo '<div id="v_save">';
                        $this->submit_button( __('Save Changes', 'wplms'), 'vibe-button-save', 'v_main_save' );
      echo '<a id="new_sample_layout" class="vibe-button-save-new-layout" style="display:none;">'. __('Save as New Layout', 'wplms').'</a>';
      echo '<a id="generated_shortcode" class="vibe-button-save-new-layout" style="display:none;">'. __('Generated Shortcode', 'wplms').'</a>';
      $shortcode = '';
      if(isset($_GET['post'])){
  ...
  public function submit_button($label,$name){
    return '<input type="submit" value="'.$label.'" name="'.$name.'" />';
  }
        ...
        if ( $change_image ) {
          ...
          echo '<div class="attachment clearfix" data-attachment="' . esc_attr( $attachment_id ) .'">' 
              ...
              . '<div class="attachment_options">'
                . '<p class="clearfix">' . '<label>' . esc_html__('Description (HTML & Shortcodes allowed)', 'wplms') . ': </label>' . '<textarea name="attachment_description[]" class="attachment_description"></textarea> </p>'
                . '<p class="clearfix">' . '<label>' . esc_html__('Link', 'wplms') . ': </label>'. '<input name="attachment_link[]" class="attachment_link" /> </p>'
              . '</div>'
              . '<a href="#" class="delete_attachment">' . esc_html__('Delete this slide', 'wplms') . '</a>'
              . '<a href="#" class="change_attachment_image">' . esc_html__('Change image', 'wplms') . '</a>'
            . '</div>';
        }
  ...
  function generate_column_options( $column_name, $paste_to_editor_id ){
      ...
      echo '<form id="dialog_settings">'
          . '<span id="settings_title">' . esc_html( ucfirst( $module_name ) . ' ' . __('Settings', 'wplms') ) . '</span>'
          . '<a href="#" id="close_dialog_settings"></a>'
          . '<p class="clearfix"><input type="checkbox" id="dialog_first_class" name="dialog_first_class" value="" class="v_option" /> ' . esc_html__('This is the first column in the row', 'wplms') . '</p>';
      
      if ( 'resizable' == $column_name ) echo '<p class="clearfix"><label>' . esc_html__('Column width (%)', 'wplms') . ':</label> <input name="dialog_width" type="text" id="dialog_width" value="100" class="regular-text v_option" /></p>';
      
      submit_button(__('Save Changes', 'wplms'), 'vibe-button-save');
    ...
    function generate_module_options( $module_name, $module_window, $paste_to_editor_id, $v_module_exact_name ){
      $v_modules = $this->v_modules;
      
      $i = 1;
      $form_id = ( 0 == $module_window ) ? 'module_settings' : 'dialog_settings';
      
      echo '<form id="' . esc_attr( $form_id ) . '">';
      echo '<span id="settings_title">' . esc_html( $v_module_exact_name . ' ' . __('Settings', 'wplms') ) . '</span>';
        ...
        switch ( $option_settings['type'] ) {
          ...
          case 'select':
            ...
            '<select name="' . esc_attr( $option_slug ) . '" id="' . esc_attr( $option_slug ) . '" class="chzn-select v_option' . $content_class . '">'
              . ( ( '' == $std ) ? '<option value="nothing_selected">  ' . esc_html__('Select', 'wplms') . '  </option>' : '' );
           ...
           case 'text':
            ...
            echo '<input name="' . esc_attr( $option_slug ) . '" type="hidden" id="' . esc_attr( $option_slug ) . '" value="" class="regular-text v_option v_upload_field' . $content_class . '" />' . '<img src="'.VIBE_URL.'/includes/metaboxes/images/image.png" class="uploaded_image" /><a href="#" rel-default="'.VIBE_URL.'/includes/metaboxes/images/image.png" class="remove_uploaded">cancel</a><a href="#" class="v_upload_button button">' . esc_html__('Upload', 'wplms') . '</a>';
            break;
           case 'slider_images':
            $std = isset( $option_settings['std'] ) ? $option_settings['std'] : '';
            echo '<div id="v_slider_images">' . '<div id="'.$std.'" class="slides v_option "></div>' . '<a href="#" id="v_add_slider_images" class="button button-primary button-large">' . esc_html__('Add Slider Image', 'wplms') . '</a>' . '</div>';
            break;      
        }
