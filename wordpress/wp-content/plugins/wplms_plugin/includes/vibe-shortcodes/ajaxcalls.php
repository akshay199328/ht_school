...
class Vibe_Shortcodes_Ajax_Calls{
    ...
    */
    function wplms_register_user(){
        ...
        if(empty($name)){
            echo '<div class="message_wrap"><div class="message">'._x('Invalid Submission, name missing','error message when name missing','wplms').'<span></span></div></div>';
                die();
        }
        $member_type = '';
        $wplms_user_bp_group = '';
        ...
        if(empty($flag)){
                            ...
                            if(isset($form_settings[$setting->id])){
                                ...
                                if($setting->id=='wplms_user_bp_group'){
                                    if(in_array($setting->value,$reg_form_settings['settings']['wplms_user_bp_group']) || $reg_form_settings['settings']['wplms_user_bp_group'] === array('enable_user_select_group')){
                                        ...
                                        echo '<div class="message_wrap"><div class="message">'._x('Invalid Group selection','error message when group is not valid','wplms').'<span></span></div></div>';
                                        die();
                                    }
            ...
            */
            ...
            if(empty($user_args['user_email']) || empty($user_args['user_pass']) || empty($check_filter)){
                echo '<div class="message_wrap"><div class="message">'._x('Invalid Email/Password !','error message when registration form is empty','wplms').'<span></span></div></div>';
                die();
            }

            //Check if user exists
            if(!isset($user_args['user_email']) || email_exists($user_args['user_email'])){
                echo '<div class="message_wrap"><div class="message">'._x('Email already registered.','error message','wplms').'<span></span></div></div>';
                die();
            }
            ...
            if(!isset($user_args['user_login'])){
                ...
                if(email_exists($user_args['user_login'])){
                    echo '<div class="message_wrap"><div class="message">'._x('Username already registered.','error message','wplms').'<span></span></div></div>';
                    die();
                }
