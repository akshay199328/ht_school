...
class WPLMS_ZIP_UPLOAD_HANDLER{
    ...
    public static function init(){
        ...
        if ( is_null( self::$instance ) )
            self::$instance = new WPLMS_ZIP_UPLOAD_HANDLER();
	...
	function wp_zip_upload_media_button($editor_id) {
		...
		if($editor_id){
			echo '<a href="'.admin_url('media-upload.php?type=upload&TB_iframe=true&tab=upload').'" class="thickbox button">
	  		 <div class="dashicons dashicons-upload"></div> '.__('Upload ZIP','wplms').'</a>';
		}
	}
	...
	function zip_tabs($tabs) {
		$newtab1 = array('zip' => __('Upload Library','wplms'));
		$newtab2 = array('upload' => __('Upload File','wplms'));
	...
	function print_detail_form($tab="upload", $file_url="", $dirname=""){
		?>
		<div class="package_upload_detail_form" style="display:none">
			<label><?php _e('Iframe source','wplms'); ?> : </label> 
			...
			?>		
			<hr />
			<label><?php _e('Package Type','wplms') ?></label> : <select name="package_type"><option value="default"><?php _e('Default (xAPI or None or Tincan)','wplms'); ?></option><option value="2004"><?php _e('SCORM 2004','wplms'); ?></option><option value="1.1"><?php _e('SCORM 1.1, 1.2','wplms'); ?></option></select>
			<label><?php _e('Maximum height of iFrame','wplms'); ?> : </label> <input type="text" name="package_upload_max_height" class="package_upload_max_height" value="800">

			<br>
			<label><?php _e('Open in Popup','wplms'); ?> : </label> <input type="checkbox" name="open_in_popup" class="package_open_in_popup" value="1" style="margin: 5px 0px;">
				
			<div  class="package_popup_button_label_div" style="display:none;">
				<label><?php _e('Popup Button label','wplms'); ?> : </label> <input type="text" name="popup_button_label" class="package_popup_button_label" value="<?php _e('Open','wplms'); ?>" style="margin: 5px 0px;">
			</div>
			<div   class="package_tincan_endpoint_div">
				<label><?php _e('Tincan Endpoint','wplms'); ?> : </label> <input type="text" name="tincan_endpoint" class="package_tincan_endpoint" value="<?php echo site_url(); ?>" style="margin: 5px 0px;">
			</div>

			<input type="button" class="button package_upload_insert_into_post"  value="<?php _e('Insert Into Post','wplms'); ?>" /> 
		</div>
	<?php
	}
	...
	function printInsertForm(){
		...
		$uploadDirUrl=$this->getUploadsUrl();
		 ...
		 <?php
		  ...
		  echo '<span>'.sprintf(__('Showing Content %d - %d from %d','wplms'),$lower_bound,$upper_bound,count($dirs)).'</span>';
			...
			echo "<table class='widefat'>";
				foreach ($dirs as $i=>$dir):
					...
					echo '<tr '.(($i%2)?'class="alternate"':'').'>
							...
							echo '<span class="package_upload_controls" style="float:right">';
		...
		echo __('No packages available','wplms');
		}
	...
	function print_js(){ 
		...
		$translation_array = array( 
			'upload_zip_error'=> __( 'Please upload a Zip package with index.html file','wplms' ), 
			'no_data'=> __( 'No data found!','wplms' ), 
			'remove_package'=> __( 'Are you sure you want to remove this package?','wplms' ), 
			);
	...
	function print_upload(){
		$this->print_js();
		?>
		<form enctype="multipart/form-data" id="zip_upload_form" style="background: #fff;margin: 0 -30px;padding: 30px;" action="admin-ajax.php" method="POST">
		<input type="hidden" name="action" value="zip_upload" />
		<input type="hidden" name="MAX_FILE_SIZE" value="1000000000" />
		<div style="display: grid; grid-template-columns: 1fr 1fr; grid-gap: 10px;">
			<div style="border: 2px dashed rgba(0,0,0,0.2); padding: 15px;">
				<strong><?php _e('Choose a file to upload','wplms'); ?></strong></td><td> <input name="uploadedfile"  id="uploadedfile" type="file" /></td></tr>
			</div>
			<div>
				<div style="display:grid;grid-template-columns: 1fr 1fr;margin-bottom:10px">
					<label><?php _e('Package Type','wplms') ?></label>
					<select name="package_type"><option value="default"><?php _e('Default (xAPI or None or Tincan)','wplms'); ?></option><option value="2004"><?php _e('SCORM 2004','wplms'); ?></option><option value="1.1"><?php _e('SCORM 1.1, 1.2','wplms'); ?></option>
					</select>
				</div>
				...
				<div  style=" display:none;" class="package_popup_button_label_div">
					<label style=" width:50%;    display: inline-block;"><?php _e('Popup Button label','wplms'); ?> : </label> <input type="text" name="popup_button_label" class="package_popup_button_label" value="<?php _e('Open','wplms'); ?>" style="margin: 5px 0px;">
				</div>
				<div  style="display:grid;grid-template-columns: 1fr 1fr;justify-content: center; align-items: center;" class="package_tincan_endpoint_div">
					<label><?php _e('Tincan Endpoint','wplms'); ?> : </label> <input type="text" name="tincan_endpoint" class="package_tincan_endpoint" value="<?php echo site_url(); ?>" style="margin: 5px 0px;">
				</div>
			</div>
			<input type="submit" value="<?php _e('Upload File','wplms'); ?>" class="button" /></td>
		</div>
		</form>
		<p><i><?php _e('Please choose a .zip package file','wplms'); ?></i></p>
		<img id="media_loading" style='display:none;' src= "<?php echo WPLMS_PLUGIN_INCLUDES_URL . '/vibe-shortcodes/images/loading.gif' ;?>" /><br />
...
function media_upload_zip_form(){
	...
	echo '<h2>'.__('Upload File','wplms').'</h2>';
...
function media_upload_zip_content(){
	...
	$wplmsthis->print_tabs();
	echo '<div class="upload_directory">';
	echo '<h2>'.__('Upload Library','wplms').'</h2>';
